...
- `pi-stack config show|path|set|unset` — inspect or mutate config without touching the file; `pi-stack config get <key>` is the machine-readable single-key read half (profile-aware, space-separated lists) that the Makefile's operational targets source.
