...
| `routing.json` | → `~/.pi/agent/routing.json` (the MODEL router's compiled intent→model map; same swap-one-file pattern as capabilities.json but for models). Generated by `pix models route` from `services/host/routing/` (registry + scorecard + policy). See `docs/design/routing.md` |
