...
- **Two commands, two intents (owner decision):** `pi-stack run` = stay out of the way; `pi-stack setup` = actually set you up (guided).
  ...
  - **`setup`** (`setupHostPhase`) is the GUIDED path and can leave you able to run BOTH the sandbox AND host mode: (1) HOST phase — the same `bootstrapProviderKeys` (steer to 1Password, writing refs to BOTH `op-refs.env` (sandbox→sbx) AND `hostmode.env` (host mode's `op run --env-file`) so one paste wires both), ensure memory, create the personal pack, and a **default-NO** TTY offer to enable + provision **host mode** (provision-before-enable: `runHostSetup` then verify `hostProvisioned()` then `host.enabled=true`; `host setup` is NOT gated behind `host.enabled`); (2) AGENT handoff — launches a `run` whose first pi message (`onboardingKickoff`) puts the `onboarding` skill in **GUIDED** mode: teach the flow by doing (memory/skills/crew/knowledge/packs), co-author a real artifact into the pack, land a task. `setup` still **guards** against an existing sandbox (`setupSandboxExists`) so the handoff lands in a fresh session. The `onboarding` skill has two modes: GUIDED (from setup) and QUICK (from "onboard me"). `pi-stack onboard` is the **host-only, no-handoff** CI path (`--account`, `--knowledge`, `--mcp`, `--model`, `--yes`, `--apply`). Host config the fenced agent can't apply is PROPOSED via `.pi-stack/onboarding.json`, gated on next `run`.
