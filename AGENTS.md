...
| `agents/*.md` | 19 subagent presets: orchestration (`fanout`/`review`/`deep`) + a role crew (architect, engineer, designer, qa-lead, security-lead, …). Roster mutation is a hand-edit, not a CLI verb (`agent new/edit/rm/reassess` were removed): edit the file, then `pix models route` |
...
- **The subagent roster is `pix agent ls` only** (resolved model + WHY for each). `new`/`edit`/`rm`/`reassess` were removed: an agent is a hand-edited `agents/*.md` file, not a CLI mutation surface; edit the file, then run `pix models route` to re-resolve and recompile `routing.json`.
