...
| `services/host/cmd/pi-stack/` | **`pi-stack` launcher** — the user-facing binary. Install without cloning the repo (`make install` builds + symlinks both `out/pi-stack` and `out/pi-stack-host`). Verb tree: `run [DIR]` (default), `serve`, `doctor`, `setup`, `config show\|path\|set\|unset`, `mcp register\|ls`, `state <backup\|restore\|reset\|uninstall>`, `paths`, `migrate`, `version`, tiered `help [--all]`. Runtime config: `~/.config/pi-stack/config.toml`, managed by `pi-stack config set <key> <value>` — never hand-edit. Storage follows the XDG layout (CONFIG `~/.config/pi-stack` = config.toml + op-refs + broker-token; DATA `~/.local/share/pi-stack` = memory + knowledge bundle + backups; STATE `~/.local/state/pi-stack` = knowledge index + caches + tasks + serve.pid), all resolved through the `config` paths module (`config/paths.go`) with a read-fallback to the legacy `~/.pi-stack` + `~/.config/pi-stack/knowledge{,-cache}`. `pi-stack paths` shows the bases; `pi-stack migrate` (execs `pi-stack-host migrate`) relocates a pre-XDG install once, explicitly, deleting nothing. |
...
- `pi-stack paths` — show where data lives (the three XDG bases: config/data/state) + any active overrides + a pending-migration note when a pre-XDG install has data at a legacy location. Read-only.
- `pi-stack migrate` — relocate storage to the standard XDG layout, once and explicitly (execs `pi-stack-host migrate`, where the sqlite integrity_check + index rebuild live). Nothing precious is deleted (moved/copied+verified, legacy left as a symlink or `.pre-xdg-<ts>` safety copy); the index is rebuilt not moved; memory moves only while its flock is free.
