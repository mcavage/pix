...
- `pi-stack pack new|add|ls|show|use|rm` — the **pack**: a git-backed context bundle (skills + knowledge + reference-only integrations + config) that is now the unit of context (it REPLACED profiles). `new` adopts an existing repo or git-inits one; `add skill|knowledge` writes an artifact; `use <path|git-url>` sets the active pack (config `pack`); `run --pack` / the active pack mounts its `skills/` + `knowledge/`. See `docs/design/packs.md`.
...
**Profiles are REMOVED — the active pack is the context.** The old `profile` concept (config `[profiles.*]` override tables, `active_profile`, the `--profile` flag, `PI_STACK_PROFILE`, `config.Resolve`, the `profile` verb) is deleted. Switching work<->personal is switching the active pack (`pi-stack pack use`). Memory (:11435) stays a single shared store (the in-store scope column is retained, dormant, for a future pack-scope); knowledge scoping is still per-workspace via `.pi-stack/knowledge.scope`. `docs/design/profiles.md` is historical; `docs/design/packs.md` is current.
