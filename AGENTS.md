...
| `mcp.json` | → `~/.pi/agent/mcp.json`, read by the **pi-mcp-adapter** extension (pi isn't a native sbx agent integration, so nothing auto-wires the gateway; we do). Baked default is EMPTY (`{"mcpServers":{}}`); at sandbox create the pi-kit install hook registers the **sbx MCP gateway** into it (`mcp-gateway` → `url: $MCP_GATEWAY_URL`, `Authorization: Bearer $MCP_SENTINEL_TOKEN_NAME`), but only when `$MCP_GATEWAY_URL` is set, so a non-Docker host stays MCP-empty. Normal launches attach no backend servers: pi sees the gateway's compact `mcp-find`/`mcp-exec` surface and discovers/calls registered tools on demand. Explicit `pi-stack mcp load` remains a debugging escape hatch but expands that backend's schemas into context. See the sandboxes repo `docs/runbooks/mcp-gateway-registration.md`. |
...
- `pi-stack secret status|edit|check` — manage the 1Password `op-refs.env` references that supply MCP-server credentials (Slack, gog, pack-provided host servers). Read-mostly: it never writes a secret value to disk (values live in 1Password), only seeds/opens/validates the refs file. `status` (default) shows which refs resolve; `edit` opens `op-refs.env`; `check` verifies every ref resolves via `op`.
- `pi-stack host [DIR]` / `pi-stack host setup` — the UNSANDBOXED escape hatch: exec pi directly on the host for work the sandbox structurally cannot do (system installs, real devices like `/dev/tty*`). Gated OFF by default behind `host.enabled`, loudly signposted; `pi-stack host setup` provisions AND enables it in one step (the gate stays off unless provisioning succeeds). Setup, readiness checks, and launch require the exact `hostPinnedPiPackage` version, and readiness/launch also require the matching curated-extension lock marker from `host setup`; a missing or stale host pi is rejected before extension loading with the exact pinned install command, while stale extensions point back to `pi-stack host setup`. Not touched by `pi-stack setup`.
...
- **ALWAYS fully-qualify model ids** (`provider/id`). A bare name like `haiku` can resolve to a keyless provider (e.g. `amazon-bedrock`) and **hang the subagent forever**. Known-good (current registry, `route models`): `anthropic/claude-opus-5`, `anthropic/claude-fable-5`, `anthropic/claude-sonnet-5`, `anthropic/claude-haiku-4-5`, `openai/gpt-5.6-sol`, `openai/gpt-5.6-terra`, `google/gemini-3.1-pro-preview`, `google/gemini-3.6-flash`, `google/gemini-3.1-flash-lite`. (`anthropic/claude-opus-4-8` and `google/gemini-3.5-flash` are RETIRED — superseded by Opus 5 and Gemini 3.6 Flash, `available:false`.) These move as models ship — run the `model-refresh` skill against LIVE pricing/benchmarks rather than trusting a name from memory.
...
- **MCP runs through sbx's LOCAL data-plane gateway** — always available, **no
  ...
  active or transient pack integration, stays registered behind the gateway**.
  No `--static-mcp` set is emitted and launch does not call `sbx mcp load`.
  Pi uses the gateway's `mcp-find`/`mcp-exec` tools on demand. Servers come from
  ...
  `loaded` (see below). Explicit loading is exceptional because it expands the
  backend's full tool schema set into pi context. Add a server = a tool
...
- **Private packs + host/container integrations (company-specific).** Open-core
  boundary: nothing company-specific is in the public repo, and **no `pi-stack-host`
  ...
  - **A pack** (git-backed, runtime, no build): carries private `skills/`,
    ...
    root that mounts to `~/.pi/agent/capabilities.json`. `pi-stack pack use <path>`
