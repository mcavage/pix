...
- **Onboarding is TWO explicit commands (owner decision), not an auto-offer inside `run`** (`docs/design/onboarding.md`; impl in `setup.go`). `pi-stack run` NEVER onboards on its own: on an unconfigured host it prints a one-line, non-blocking heads-up (`warnUnconfigured`) and goes straight into the agent — no prompt, no delay. `pi-stack setup` is the **guided** path, two phases: (1) HOST phase (`setupHostPhase`) reports provider keys with the exact `sbx secret set` fix for any missing, ensures memory, and — on a TTY with no flags — interactively wires optional knowledge + gog account (flags/CI skip the prompts); (2) AGENT handoff: it launches `pi-stack run` with an **initial pi message** (`onboardingKickoff`, forwarded as a `--` passthrough positional) that makes the agent PROACTIVELY run the `onboarding` skill and land the user in a working session. The initial-message handoff REPLACES the old passive `.pi-stack/onboarding.offer` marker + system-prompt injection, which never spoke until the user typed (that was the "dumped into a silent agent" bug). Because the kickoff only lands cleanly in a FRESH session, `setup` has a **guard** (`setupSandboxExists` on `probeTaskSandbox`): if a sandbox already exists for the dir it REFUSES up front (before touching host config) and tells you to `pi-stack rm <name> && pi-stack setup`, or `pi-stack run`, or `onboard me` in the running agent — rather than half-onboarding into a re-attached silent session. An unresolvable name/profile or an unprobeable sbx does NOT block (only a positively-present sandbox does). `pi-stack onboard` is the **host-only, no-handoff** path for CI (`--account`, `--knowledge`, `--mcp`, `--model`, `--yes`, `--apply` to reconcile a pending `.pi-stack/onboarding.json` — allowlist-validated, diffed, `[Y/n]`-gated). Two trust planes: identity → memory (live), host config → declarative file applied under a gate (NO new listener; the fenced VM can only PROPOSE within a typed schema). `setup` is a real command (no longer a deprecated alias); the old 4-step wizard is DELETED.
