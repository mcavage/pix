...
`pi-stack` (`services/host/cmd/pi-stack/`) is the user-facing binary; `make install` builds and symlinks both it and `pi-stack-host`. **The verb/flag reference is NOT here.** Read it where it cannot go stale:

- **`pi-stack help --all`** (or `help <verb>`, `help --man`) — the live verb tree, generated from the dispatch switch. This is the source of truth.
...
1. `~/.config/pi-stack/config.toml` is the SINGLE runtime config and is managed by `pi-stack config set`/`unset` — **never hand-edit it**. `Save()` persists only explicit deviations from defaults, so a future default change still reaches users.
2. **`pi-stack` with no args never launches a sandbox.** It prints status. Launching is explicit, behind `run`. A bare positional launches `run` only when it names an existing directory.
...
4. **Serve runtime state lives in the STATE dir** (`~/.local/state/pi-stack/`: `serve.pid`, `serve.lazy`, `serve.spawn.lock`, `serve.log`), not the config dir, so `pi-stack reset` (which moves the config dir aside) can never orphan a running daemon from its pidfile.
...
9. **Host mode (`pi-stack host`) is the unsandboxed escape hatch and is OFF by default** behind `host.enabled`. `pi-stack setup` never enables it; only `pi-stack host setup` does, and only if provisioning succeeded (exact pinned pi version + curated-extension lock marker).
10. **`pi-stack secret` never writes a secret value to disk** — it only seeds, opens, and validates the `op://` refs file. Values stay in 1Password.
11. **`pi-stack monitor` binds loopback by default.** `--bind 0.0.0.0` is an explicit LAN opt-in with a loud warning (no auth token).
12. **`pi-stack rm` is scoped to `pi-stack-*` sandboxes only.** It can never reach a sandbox it did not create.
