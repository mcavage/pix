...
| `mcp.json` | → `~/.pi/agent/mcp.json`. **MCP is OPT-IN: the public default is EMPTY** (`{"mcpServers":{}}`), so a fresh non-Docker host never errors on an unreachable gateway at startup. The sbx Cloud MCP Gateway (`mcp-gateway.docker.internal`; atlassian/notion/granola/gog/…) is Docker-internal, so it lives in a **private pack's** `mcp.json`, not the public image. Opt in by activating a pack that ships the gateway entry, or add servers via `pi-stack mcp`. |
