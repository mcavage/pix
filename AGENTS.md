...
| `evals/` | promptfoo eval home: `promptfooconfig.yaml`, `providers/pi.js` (invokes models through `pi`), `suites/*.yaml` (+ `suites/agents/` per-agent), `asserts/` (external JS graders). `pi-stack evals run` runs it and imports results into the scorecard. Replaced the hand-rolled Go scorers |
