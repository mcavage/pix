...
- **Two commands, two intents (owner decision):** `pi-stack run` = stay out of the way; `pi-stack setup` = actually set you up (guided).
  ...
  - **`setup`** (`setupHostPhase`) is the GUIDED path and sets up BOTH the sandbox AND host mode: (1) HOST phase — `setupProvisionKeys` steers model keys to 1Password (the preferred source, regardless of what sbx has; degrades to existing sbx keys when `op` is absent or a provider is skipped): solicits an op:// ref per provider without one, mirrors to `hostmode.env`, force-syncs ALL provider refs into sbx with `secret set -f`, no yes/no gate, ensures memory, creates the personal pack, `seedIdentity` reads git `--global` identity into host-state + memory, and `setupHostMode` provisions + enables **host mode** with no prompt WHEN provisioning succeeds (provision-before-enable: `runHostSetup` then verify `hostProvisioned()` incl. `pi` on PATH, then `host.enabled=true`; disables a stale gate if provisioning fails; `host setup` is NOT gated behind `host.enabled`); (2) AGENT handoff — launches a `run` whose first pi message (`onboardingKickoff`, a short human line) drops into the `onboarding` skill: teach the flow BY DOING the user's real task, introducing each capability (memory/skills/crew/packs) only when the work needs it. `setup` still **guards** against an existing sandbox (`setupSandboxExists`) so the handoff lands in a fresh session. `pi-stack onboard` is the **host-only, no-handoff** CI path (`--account`, `--knowledge`, `--mcp`, `--model`, `--yes`, `--apply`). Host config the fenced agent can't apply is PROPOSED via `.pi-stack/onboarding.json`, gated on next `run`.
