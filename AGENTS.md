...
- **Nightly kit-schema skew fails before mutation.** For an unreleased/local checkout, `pix setup` and fresh `pix run` ask the installed `sbx kit validate` to parse every local kit before opening OAuth browsers, changing host state, or invoking `sbx run`. A failing validator means the installed `sbx@nightly` is older than the checkout's kit schema; update/rebuild sbx rather than rewriting a valid current-tree spec to fit the stale binary.
...
- **A stale Pix service is repaired by identity, never by port alone.** Before lazy service startup, `pix run` probes the enabled service's `identity` response. A positively identified `pix-memory`/`pix-knowledge` at an older launcher version is restarted through its lifecycle owner (managed supervisor or safe lazy stop); foreign or unverifiable port holders are never killed, and foreground service processes remain the terminal owner's responsibility.
