...
- **Model routing (cost/latency/accuracy).** Agents no longer hard-pin a `model:`; they declare an **`intent:`** (`max-accuracy`, `code`, `reasoning`, `review`, `breadth`, `verify`) and the router resolves it to a concrete model. An intent is a **hard-constraint problem**: filter models by cost/latency ceilings + accuracy floor + provider allowlist, then optimize the objective. Truth lives on the HOST (Go, `services/host/routing/`): `models.json` (registry + prices — **adding a model is one entry**), `scorecard.json` (measured accuracy/cost/latency, seeded then earned by evals), `policy.json` (the intents). `pi-stack route compile` resolves every intent and writes **`routing.json`** (baked next to `capabilities.json`, same swap-one-file pattern); `subagents.ts` reads it **offline** (never calls the host at spawn — no hang risk). An explicit `model:` still wins (back-compat); an unknown/missing intent degrades to "inherit the parent model". The interactive session can route too: `pi-stack run --intent code`. **Evals** (`pi-stack evals run`) measure accuracy with pluggable scorers (`contains`/`regex`, `command` = build/test, `judge` = LLM rubric), record real cost+latency, and are **budget-guarded + `--dry-run`** — a manual, on-new-model-release sweep, NOT an unattended spender (the whole point is to stop burning money). Add a model: entry in `models.json` → `pi-stack evals run --models <id>` → `pi-stack route compile` → `make load`. No agent files change. Full design: `docs/design/routing.md`.
