...
- **Private packs + host/container integrations (company-specific).** Open-core
  ...
  recompile is ever needed.** The build-time `OVERLAY=..` peer-repo model (mixin kit
  + compiled-in `host/overlay_*.go`) is **RETIRED** — see **`docs/OVERLAY.md`**.
  Private context ships three ways:
