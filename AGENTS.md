...
- **Two commands, two intents (owner decision):** `pi-stack run` = stay out of the way; `pi-stack setup` = actually set you up (guided).
  ...
  - **`setup`** (`setupHostPhase`) is the GUIDED path and sets up BOTH the sandbox AND host mode: (1) HOST phase — `setupProvisionKeys` PREFERS 1Password for every provider key (Anthropic/OpenAI/Google) but is not mandatory: `op` missing/not-signed-in is a HARD failure on the 1Password path (never a silent degrade to raw sbx keys), an existing ref is confirmed but must still resolve, a missing ref is prompted for once on a TTY (or fails with the exact `pi-stack secret set` command otherwise), every ref is mirrored to `hostmode.env`, and sbx is reconciled to match — an sbx value that's changed/unknown is confirmed in ONE BATCHED `[Y/n]` prompt (default yes: 1Password is the source of truth), decline or non-interactive-without-`--yes` FAILS setup rather than reporting success with mismatched sources. Ordinary value flags (`--account`/`--knowledge`/`--mcp`/`--model`) do NOT suppress these prompts; only `--yes`/`-y`/`--non-interactive` does, and `--yes` alone does NOT skip 1Password. **Three ways to skip 1Password for a run, trusting sbx's existing keys instead:** the explicit `--use-sbx-keys` flag (works interactively or headless, wins even if provider refs already exist, never deletes a ref or synced record, it just isn't used this run); a **persisted `provider_key_mode=sbx`** in config.toml from a prior successful run (auto-skips with no prompt, still re-runs the exact all-three probe every time — never a cached bypass); or the one-time interactive convenience prompt (`sbxKeysConveniencePrompt`) offered ONLY when interactive, not `--yes`, mode UNSET, sbx's exact probe (`sbxAllModelKeysPresent`) reports all three keys present, AND no provider ref is configured yet (`probeProviderKeyRefs`, a tri-state none/some/error read that fails setup outright on a genuine read error instead of masking it as "none"). Default Enter is yes, declining falls through to the strict flow with no further retries, and once any ref exists, a mode is persisted, or the run is non-interactive, this prompt never appears. The explicit `--use-1password` flag is the mutually exclusive opposite: it forces the strict flow for one run even over a persisted sbx mode or a complete sbx. All three skip paths require the EXACT successful sbx probe; absent/erroring/incomplete sbx fails with a clear message naming exactly which provider(s) are missing (`acceptExistingSbxKeys`). The skip path prints one compact status line instead of the full `reportProviderKeys` listing (which only runs on the strict path, to avoid a noisy report immediately followed by a prompt), and it only claims the sandbox now uses existing sbx keys with 1Password reconciliation skipped — it never speaks for host mode's cloud-key status; `setupHostMode` owns that ONE status line, reporting cloud keys as "validated this run" only after the strict flow actually resolved them via `op read`, or "configured (not verified this run)" when a skip path supplied them instead. Whichever mode succeeds (`sbx` or `1password`) is a typed `providerKeySetupResult`, never a package global, and is persisted to config.toml's `provider_key_mode` on success; a mode-save failure fails setup honestly. Then it ensures memory, creates the default pack, `seedIdentity` reads git `--global` identity into the trusted host-state payload (injected directly into the launcher-generated kickoff prompt at `run` time, never written to a workspace file) + memory, and `setupHostMode` provisions + enables **host mode** with no prompt WHEN provisioning succeeds (provision-before-enable: `runHostSetup`/`hostProvisioned` — swappable seams so tests never touch real host state or make real `pi install` network calls — then `host.enabled=true`; disables a stale gate if provisioning fails; `host setup` is NOT gated behind `host.enabled`). When a run skipped 1Password and `hostmode.env` has no provider refs, `setupHostMode` reports that as a legitimate local/Ollama-only result, not a should-not-happen defensive branch, and never asks for refs later in the same setup run; (2) AGENT phase depends on whether a sandbox already exists for DIR — the host phase above ALWAYS runs first either way (repeat `pi-stack setup` reconciles keys/config again). Positively ABSENT: launches a `run` whose first pi message (`onboardingKickoff`, a short human line) drops into the `onboarding` skill for a ONE-SHOT upfront tour (not progressive/teach-as-you-go). EXISTING without `--replace`: left alone (never force-removed, never replayed into a live session) — setup reports success and prints `pi-stack run [DIR]` (reattach; keeps create-time pack/MCP/skills) / `pi-stack setup [DIR] --replace` (recreate with current settings + get the tour; setup's own `--replace` flag relaunches through `run --replace` carrying the kickoff, harmless when absent). UNKNOWN state (sbx errored): FAILS CLOSED after the host phase — never launches blind, tells you to fix sbx and retry. `pi-stack onboard` is the **host-only, no-handoff** CI path (`--account`, `--knowledge`, `--mcp`, `--model`, `--yes`, `--apply`). `--use-sbx-keys` and `--use-1password` are setup-only (mutually exclusive when both given) because onboard never provisions provider keys. `pi-stack secret set` for one of the three provider keys mirrors it into BOTH `op-refs.env` and `hostmode.env`, so three `secret set` commands really are enough to wire both the sandbox and host mode. Host config the fenced agent can't apply is PROPOSED via `.pi-stack/onboarding.json`, gated on next `run`.
