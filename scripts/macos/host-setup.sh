#!/usr/bin/env bash
# pix macOS host setup — guided, one step at a time.
#
...
#
# Nothing here is full-auto: the whole point of pix is that full-auto lives
...
if ask "Run the reftest sandbox now?"; then
  if sbx run pix --name pix-reftest --kit "git+https://github.com/mcavage/pix.git#ref=main&dir=pi-kit" . ; then
    ok "Booted → #ref works; version-coupled 'pix run' is good."
  else
    warn "sbx rejected #ref. Fallback: pin the image via --template (see cmd/pix/sbxargs.go TODO)."
  fi
  sbx rm -f pix-reftest >/dev/null 2>&1 || true
else info "Skipped — but verify before relying on 'pix run' version pinning."; fi
...
# The plist is no longer copied/sed'd from this repo: `pix serve install`
# renders it from the template embedded in the launcher (the single source of
# truth, services/host/cmd/pix/templates/) and bootstraps it via launchctl.
bold "\n[4] Run host services as a managed login service (memory :11435)"
info "Installs ~/Library/LaunchAgents/com.pix.serve.plist (RunAtLoad + KeepAlive)."
if ask "Install the managed service now (pix serve install)?"; then
  PI="$HOME/.local/bin/pix"; [ -x "$PI" ] || PI="$ROOT/out/pix"
  if "$PI" serve install; then ok "managed service installed — logs at ~/Library/Logs/pix-serve.{out,err}.log"; else warn "pix serve install failed"; fi
  info "Remove later:  pix serve uninstall"
else info "Skipped. Lazy auto-start covers you (pix run/memory start serve on demand), or run:  pix serve"; fi

# --- Step 5: wire your accounts (interactive) ----------------------------------
bold "\n[5] Wire your accounts — setup + doctor"
info "setup seeds ~/.config/pix/config.toml and prompts for missing secrets/ollama/mcp."
if ask "Run 'pix setup' now?"; then pix setup || warn "pix setup returned nonzero"; else info "Skipped."; fi
if ask "Run 'pix doctor' to confirm?"; then pix doctor || true; else info "Skipped."; fi
...
info "  - run 'pix config set mcp gog' so 'make run' / 'pix run' attaches it"
