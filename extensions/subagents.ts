...
// ─── Model roster (literal main/agent table, generated by the host) ─────────
// The host composes the selected environment's literal `[models].main` +
// `[agents]` table into the SAME generated inference.json every reader
// already parses (docs/design/environments.md §5.2/§6.4/§7) — there is no
// second generated routing artifact that can disagree with provider
// registration. `roster` is additive: an absent or malformed roster is
// indistinguishable from "no environment roster is selected", so a manifest
// an older host wrote, or written with no environment active, behaves
// exactly as it always has (every agent falls through to inherit the parent
// model, unless it names its own `model:`).
//
// Precedence (§6.4): explicit `model:` on a custom agent file wins outright;
...
function loadRoster(): Roster | undefined {
	try {
		...
		return parseRoster(JSON.parse(fs.readFileSync(p, "utf-8")));
	} catch {
		/* best-effort; a missing/unreadable/unparseable file just means "no roster" */
		return undefined;
	}
}
const ROSTER = loadRoster();
// isQualifiedModelId guards every model id this file ever forwards to a
// child `pi --model`: a bare (non `provider/id`) name can resolve to a
// keyless provider and hang, so a roster entry that fails this check is
// treated as absent rather than forwarded.
function isQualifiedModelId(id: string): boolean {
	const i = id.indexOf("/");
	return i > 0 && i < id.length - 1;
}
// resolveRosterModel normalizes one roster lookup (`roster.main` or
// `roster.agents[name]`): undefined/blank/unqualified all collapse to
// `undefined`, so every call site can chain `??`/`||` without re-checking
// shape.
function resolveRosterModel(id: string | undefined): string | undefined {
	const v = id?.trim();
	return v && isQualifiedModelId(v) ? v : undefined;
}
...
// an unavailable cloud backend — the one exception that stays with the
// Ollama bridge transport (§4.1/§6.4) rather than the roster: it overrides
// even an explicit `model:` or roster entry. Cloud parent models keep normal
// roster resolution.
			...
			handler: async (rawArgs: any, ctx: any) => {
				...
				if (arg.startsWith("doctor")) {
					...
					const canaryModel =
						...
						resolveRosterModel(ROSTER?.main) ||
						currentModel;
				...
				lines.push(
					`roster: ${ROSTER ? `on (main=${ROSTER.main}, ${Object.keys(ROSTER.agents).length} agent override${Object.keys(ROSTER.agents).length === 1 ? "" : "s"}, inference.json)` : "off (no roster in inference.json; agents use model:/inherit)"}`,
				);
