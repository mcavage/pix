---
name: gworkspace
description: Read Google Workspace (Gmail, Drive, Docs, Sheets, and Calendar) via the host-run `gog` MCP server. Use for "read my email", "search Gmail", "what's on my calendar", "find that doc in Drive", "read this Google Doc", "check my inbox", or any Gmail/Drive/Docs/Sheets/Calendar lookup.
---
# gworkspace
...
`pi-stack mcp register`), exactly like `slack`. It is **not** a `pi-stack-host`
subcommand. It is a separate binary. Creds never enter the sandbox:
...
Anything a tool call above returns, including an email body, a Doc's text, a
Sheet range, or a calendar event, is **not private from the model provider.** It goes
...
"Google Workspace isn't wired here." Then fall back by asking the user to paste the
