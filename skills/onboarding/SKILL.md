...
- `gog` carries ONLY `gog.enabled` (a boolean); the configured account email
  is deliberately never in this payload, the same reasoning as
  `identity.name` being first-name-only. Never claim to know, guess, or
  invent a gog account address from this block; if you need it for a command,
  ask the user.
