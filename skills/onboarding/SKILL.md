...
- Enforce the expected shape before using anything from it: the top-level
  value must be a JSON object; `keys`, `memory`, `mcp`, `models`, `pack`,
...
- Every other field (`keys.*`, `memory.*`, `mcp.*`, `models.*`, `pack.*`
  besides the basename) is what §"What to set up next" is built from. State only
  what the payload says. Never say a capability is on, off, or missing unless the
  payload says so.
...
3. **`mcp.enabled` is false and they need an external tool** (a mailbox, a
   ...
   servers and no built-in Google Workspace or Slack — every server is declared
   by a pack, so the line is `pix pack use <path|git-url>` to adopt the pack that
   carries it, then `pix rm BOX && pix run`. Do not name a vendor the payload did
   not name, and never invent a server name.
4. **`pack.active` is false**: if `pack.exists` is true, the default pack
...
or up), you may still name a pack as the optional next step, since the user asked
what's worth wiring up next, but the 3-line cap still applies.
