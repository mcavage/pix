...
- Enforce the expected shape before using anything from it: the top-level
  value must be a JSON object; `keys`, `memory`, `mcp`, `models`,
  `identity` are objects with the field names below, of the documented types
  (booleans stay booleans, string fields stay strings, array fields stay arrays
  of strings). If the block is missing, malformed, not valid JSON, or any field
  is the wrong type, treat it exactly like "no payload" above: skip identity and
  setup-gap claims rather than guessing or partially trusting it.
...
- Every other field (`keys.*`, `memory.*`, `mcp.*`, `models.*`) is what
  §"What to set up next" is built from. State only what the payload says.
  Never say a capability is on, off, or missing unless the payload says so.
...
6. **Environments, one line, only if relevant.** Pix v2 has no pack: an
   environment (`~/.pix/envs/<name>`, a plain `.sbxenv.yaml` plus optional
   `pix.toml`) is the unit that carries MCP servers, skills, and config, and
   the payload does not currently report which one is active, so do not
   invent a name or a status for it here. If the user asks, point them at
   `pix env` for the list and `pix env trust NAME` for the host-execution
   review a new environment needs once.
...
2. **Memory is enabled but stopped** (`memory.enabled` is true and
   `memory.up` is false): `pix setup` reconciles the `pix-memory` container.
   If memory is disabled, do not call it broken or tell the user to start it.
3. **`mcp.enabled` is false and they need an external tool** (a mailbox, a
   wiki, an HR directory): optional, phrase it conditionally. Pix ships no MCP
   servers and no built-in Google Workspace or Slack — every server is a
   native `.sbxenv.yaml` declaration on the environment they use, reviewed
   once with `pix env trust NAME`. Do not name a vendor the payload did not
   name, and never invent a server name.

If there are no required gaps (inference resolved, and memory either disabled
or up), there is nothing further required to name; the 3-line cap still applies
if a future payload field adds one.
...
value (an account email, a server name) you don't actually have.
