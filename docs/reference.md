...
the chosen model. Environments declaring inference backends or setup hooks
retain their own connection flow without this personal-provider interview.

The model picker offers every available shipped OpenAI, Anthropic, and Google
catalog entry, plus supported models listed by your Ollama endpoint. It does
not silently upgrade a saved model or download a different Ollama model.
Image downloads announce themselves before Docker starts pulling.
