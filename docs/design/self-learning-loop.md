...
- **Always-on service** (implemented): the store runs under `pi-stack serve`; `pi-stack serve install` registers a managed login service (launchd LaunchAgent on macOS, systemd `--user` on Linux), and `run` / `memory` / `knowledge` lazy-auto-start a detached serve when the ports are down (opt out with `PI_STACK_NO_AUTOSERVE`). See docs/design/serve-lifecycle.md.
