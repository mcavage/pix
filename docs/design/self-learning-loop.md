...
- **Promotion (step 4).** Synthesis (`store.synthesize`, distills captures and dedupes semantically) is on-demand only now — the periodic ticker that ran it on a timer was removed (U1-delete-go: no background deletion). What's not built is *promotion*: graduating a recurring lesson into a proposed, gated edit to a skill or convention. The original "staging area" collapsed into direct capture plus semantic de-dup for v1.
