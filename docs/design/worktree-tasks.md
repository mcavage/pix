...
Run several pi-stack sandboxes at once, each on a different task on the SAME
...
### Golden path
```
pi-stack task new fix-login       # clone + branch pi-stack/fix-login + sandbox, you are in it
pi-stack task new refactor-db      # a second, fully isolated, in parallel
pi-stack task ls                   # both: branch, sandbox status, git state
pi-stack task rm fix-login         # tear down when pushed/merged (guarded)
```
...
```
CO   = $STATE/pi-stack/tasks/<repokey>/co/<name>
META = $STATE/pi-stack/tasks/<repokey>/meta/<name>.json
git clone --local MAINROOT CO
git -C CO checkout -q -b pi-stack/<name> "<REF|HEAD>"
...
`deriveSandboxName` is basename-only (`pi-stack-fix` collides across repos), so
...
- **new**: clone -> checkout -b -> write meta -> launch. If `CO` already exists,
  error ("task exists; `pi-stack run <CO>` to reattach"). If launch fails after
...
- **rm** `<name>`: guarded. Refuses (exit 2, teachable, unless `--force`) when:
  ...
  3. would-lose-work: `UNREC>0 && UNPUSHED>0`, where `UNREC` = commits on
     `pi-stack/<name>` not reachable from any ref in MAINROOT (fetch into a temp
  ...
  MAINROOT fetch CO pi-stack/<name>:refs/pi-stack/recovered/<name>`), then `sbx
...
## Storage (does not touch the tracked .pi-stack/knowledge contract)

Task clones live OUTSIDE the repo at `$XDG_STATE_HOME/pi-stack/tasks/<repokey>/`
(`meta/<name>.json`, `co/<name>/`). So no file under the repo's `.pi-stack/` is
created, the tracked `.pi-stack/knowledge` pointer is untouched, no blanket
`.gitignore *` is needed, and nested-worktree hazards do not exist. Each clone
carries its own fresh untracked `.pi-stack/` (knowledge.scope / profile) — per-
...
- `main.go`: `case "task": runTask(args[1:])`. `help.go`: `knownVerbs["task"]` +
  `verbUsage` case + `taskUsage`. `pi-stack.1`: one `.SS task` block (quoted
  `"pi-stack task new"` etc; man 1:1 invariant held; new prose examples UNQUOTED
  in .nf/.RS).
...
- **v2 (defer)**: sbx `--clone` flag with fetch-before-rm recovery; recursive
  ...
  memory tagging beyond the free per-clone `.pi-stack/` split.
...
pi-stack task new probe --from HEAD -- -p 'run: git status; git commit --allow-empty -m t; git log --oneline -1'
#   MUST show clean git status + a successful commit INSIDE the sandbox
pi-stack task ls        # probe present, sandbox status shown
pi-stack task rm probe  # guarded teardown; clean case removes clone + sandbox
```
