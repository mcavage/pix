...
| **Your `web_search` query text** (and the fetched pages' contents come back through it) | `api.exa.ai` / `mcp.exa.ai` — the **zero-config default** backend. `api.parallel.ai` (Parallel search/extract) — not a last-resort fallback: the provider auto-select order checks it before Perplexity or Gemini, so supplying only a Parallel key makes it the **preferred** backend once Exa is unavailable. Other keyed backends, tried in order after Parallel: `api.perplexity.ai`, `generativelanguage.googleapis.com` (Gemini) | The `web_search`/`fetch_content` tools in `pi-web-access` | Governed by that search provider |
