...
var l3Debt = map[string]int{
	// launch prints degraded-launch warnings (a pack that vanished, an
	// unconnected integration, a receipt it could not clear) from deep inside
	// the launch path, where the caller's writer is not threaded yet.
	"workflow/launch": 6,
