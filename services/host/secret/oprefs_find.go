// oprefs_find.go — locating an EXISTING secrets.env, the companion question to
...
//
// There is exactly ONE candidate: <PIX_HOME>/secrets.env (AGENTS.md safety
// invariant 1 — PIX_HOME is the single root, with no XDG split). The
// $PIX_CONFIG directory, the repo-checkout walk (Makefile + config/
// secrets.env), and ~/.config/pix are all DELETED, not deprecated: a
...
// FindOpRefs returns the absolute <PIX_HOME>/secrets.env when that file
