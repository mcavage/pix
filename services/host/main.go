...
// shared handshake. kind is memory|knowledge|mcp (mcp also needs a <name>).
func runPlugin(args []string) {
	if len(args) < 1 {
		fmt.Fprintln(os.Stderr, "pix-host plugin: missing <kind> (memory|knowledge|mcp)")
		os.Exit(2)
	}
	switch args[0] {
	case "memory":
	...
	default:
		fmt.Fprintf(os.Stderr, "pix-host plugin: unknown kind %q (memory|knowledge|mcp)\n", args[0])
		os.Exit(2)
	}
}
