...
// TestExternalPackage_CompileInferenceRuntimeAndSynthesizeInferenceKit is
...
// straight to CompileInferenceRuntime / SynthesizeInferenceKit — the only
...
func TestExternalPackage_CompileInferenceRuntimeAndSynthesizeInferenceKit(t *testing.T) {
	...
	_, manifest, err := inference.CompileInferenceRuntime(cfg, time.Now(), roster)
	if err != nil {
		t.Fatalf("CompileInferenceRuntime() error = %v", err)
	}
	if len(manifest.Models) == 0 {
		t.Fatal("CompileInferenceRuntime() produced no models from a one-backend, one-model config")
	}
