...
func TestRenderPlist(t *testing.T) {
	...
	for _, want := range []string{
		"<string>com.pi-stack.serve</string>",
		"<string>/home/u/.local/bin/pi-stack-host</string>",
		"<string>serve</string>",
		"<key>RunAtLoad</key>",
		"<key>KeepAlive</key>",
...
func TestRenderUnit(t *testing.T) {
	...
	if !strings.Contains(got, "ExecStart=\"/usr/local/bin/pi-stack-host\" serve") {
...
func TestLaunchdInstall(t *testing.T) {
	...
	if err := launchdInstall(r.run, f.fs(), 501, "/Users/u", "/opt/pi-stack-host", nil, &out); err != nil {
		t.Fatal(err)
	}
	plistPath := "/Users/u/Library/LaunchAgents/com.pi-stack.serve.plist"
	...
	want := []string{
		"launchctl bootout gui/501/com.pi-stack.serve",
		"launchctl bootstrap gui/501 " + plistPath,
		"launchctl kickstart -k gui/501/com.pi-stack.serve",
	}
...
func TestLaunchdInstallFallsBackToLoad(t *testing.T) {
	...
	f := newRecFS()
	if err := launchdInstall(r.run, f.fs(), 501, "/Users/u", "/opt/pi-stack-host", nil, &bytes.Buffer{}); err != nil {
		t.Fatal(err)
	}
	joined := strings.Join(r.calls, "\n")
	if !strings.Contains(joined, "launchctl load -w /Users/u/Library/LaunchAgents/com.pi-stack.serve.plist") {
...
func TestLaunchdUninstall(t *testing.T) {
	...
	if len(f.removed) != 1 || !strings.HasSuffix(f.removed[0], "com.pi-stack.serve.plist") {
...
func TestLaunchdActiveAndRestart(t *testing.T) {
	...
	if r3.calls[0] != "launchctl kickstart -k gui/501/com.pi-stack.serve" {
...
func TestSystemdInstall(t *testing.T) {
	...
	f := newRecFS()
	var out bytes.Buffer
	if err := systemdInstall(r.run, f.fs(), "/home/u", "/usr/bin/pi-stack-host", nil, &out); err != nil {
		t.Fatal(err)
	}
	unitPath := filepath.Join("/home/u", ".config", "systemd", "user", "pi-stack-serve.service")
	...
	for _, want := range []string{
		...
		"systemctl --user enable --now pi-stack-serve.service",
	} {
...
func TestSystemdInstallNoSystemd(t *testing.T) {
	...
	f := newRecFS()
	err := systemdInstall(r.run, f.fs(), "/home/u", "/usr/bin/pi-stack-host", nil, &bytes.Buffer{})
	if err != errNoSystemd {
...
func TestSystemdUninstall(t *testing.T) {
	...
	joined := strings.Join(r.calls, "\n")
	if !strings.Contains(joined, "systemctl --user disable --now pi-stack-serve.service") ||
...
func TestCapturedServeEnv(t *testing.T) {
	t.Setenv("PI_STACK_CONFIG", "/custom/place/config.toml")
	...
	if len(env) != 3 {
		t.Fatalf("env = %v, want PI_STACK_CONFIG + 2 set vars", env)
	}
	if env[0].Key != "PI_STACK_CONFIG" || env[0].Value != "/custom/place/config.toml" {
		t.Errorf("env[0] = %v, want the absolute PI_STACK_CONFIG pin", env[0])
	}
...
// A custom PI_STACK_CONFIG + port override lands in the rendered plist.
...
func TestRenderUnitCarriesCapturedEnv(t *testing.T) {
	...
	for _, want := range []string{
		`Environment="PI_STACK_CONFIG=/custom/config.toml"`,
...
func TestSystemdQuoteEscapesPercentAndDollar(t *testing.T) {
	got := systemdQuote("/home/user%id/$HOME/pi-stack-host")
	want := `"/home/user%%id/$$HOME/pi-stack-host"`
	if got != want {
...
func TestRenderUnitEscapesPercentInHostBin(t *testing.T) {
	got, err := renderUnit(unitData{HostBin: "/home/user%id/pi-stack-host"})
	if err != nil {
		t.Fatal(err)
	}
	want := `ExecStart="/home/user%%id/pi-stack-host" serve`
	if !strings.Contains(got, want) {
