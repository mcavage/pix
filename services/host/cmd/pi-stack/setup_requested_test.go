...
//
//	`pi-stack setup --pull-models` with Ollama down exits 1. `pi-stack setup`
...
// in `pi-stack help setup` AND the man page.
const requestedExitContract = "pi-stack setup --pull-models with Ollama down exits 1. " +
	"pi-stack setup with the same Ollama down exits 0 with an optional ⚠ row. " +
...
func TestRequestedExitContract_InHelpAndMan(t *testing.T) {
	if !strings.Contains(normalizeCopy(setupUsage), requestedExitContract) {
		t.Errorf("`pi-stack help setup` must state the exit contract verbatim:\n%s", requestedExitContract)
	}
	b, err := os.ReadFile("pi-stack.1")
	if err != nil {
		t.Fatal(err)
	}
