...
func TestReleaseVersionsLockstep(t *testing.T) {
	...
	specMatch := regexp.MustCompile(`image:\s*"docker\.io/[^"]*/pi-stack:([^"]+)"`).FindSubmatch(spec)
	if specMatch == nil {
		t.Fatal(`pi-kit/spec.yaml: no pinned "docker.io/.../pi-stack:<version>" image tag found`)
	}
