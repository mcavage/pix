...
// `run` (the warnings printed before a launch), `status`/bare `pi-stack` (the
...
// ollama, models, pack, gworkspace, per-server MCP — belongs to `pi-stack
...
// launchWarningLimit is how many readiness rows `pi-stack run` may print
