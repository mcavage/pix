...
// discoverable in `pi-stack help --all`, or explicitly listed as hidden with a
...
func TestHelpListsEveryTopLevelVerb(t *testing.T) {
	for _, verb := range dispatchVerbs(t) {
		...
		if !strings.Contains(helpAllText, verb) {
			t.Errorf("verb %q is dispatched but absent from `pi-stack help --all` (add it, or add it to hiddenVerbs with a reason)", verb)
		}
	}
}
	...
	for verb, subs := range map[string][]string{
		...
		for _, sub := range subs {
			if !strings.Contains(usage, sub) {
				t.Errorf("`pi-stack %s %s` is dispatched but missing from its usage text:\n%s", verb, sub, usage)
			}
		}
	}
}
...
func TestExitMatrix(t *testing.T) {
	core := func(v verdict) check {
		return check{label: "core", requirement: requirementCore, verdict: v, evidence: "fixture", todo: "pi-stack setup"}
	}
	opt := func(v verdict) check {
		return check{label: "opt", requirement: requirementOptional, verdict: v, evidence: "fixture", todo: "pi-stack setup"}
	}
...
func TestRequestedPromotionBlocks(t *testing.T) {
	builders := map[Axis]axisBuilder{
		axisGworkspace: func() []check {
			return []check{{label: "gworkspace", requirement: requirementOptional, verdict: verdictTodo, evidence: "not authed", todo: "pi-stack gworkspace setup"}}
		},
	}
