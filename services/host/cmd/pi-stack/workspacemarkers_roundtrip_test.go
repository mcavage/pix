...
// ".pi-stack" -> ".pix" and this file must stay green with only the literal
...
// lives:
//
//	.pi-stack/profile               — Go writes (pack.go writeMemoryScope),
//	                                   TS reads (memory-recall.ts,
//	                                   memory-capture.ts)
//	.pi-stack/knowledge.scope        — Go writes (run.go writeKnowledgeScope),
//	                                   TS reads (knowledge-recall.ts)
//	.pi-stack/ollama-bridge.model    — Go writes (run.go writeOllamaBridgeFile),
//	                                   TS reads (ollama-bridge.ts)
//	.pi-stack/sandbox.pack           — Go writes AND reads
//	                                   (writeSandboxPackMarker /
//	                                   readSandboxPackMarker); no TS reader
//	.pi-stack/knowledge               — Go writes AND reads
//	                                   (knowledgeUseProject / readProjectPointer
//	                                   / projectBundle); no TS reader
//	.pi-stack/onboarding.json         — the IN-SANDBOX AGENT writes it (not Go);
//	                                   Go reads + removes it (reconcileOnboarding)
//	.pi-stack/host-state.json         — NEVER a file on EITHER side, by design
//	                                   (hoststate.go); this file's job is to
//	                                   prove that stays true even after every
//	                                   OTHER marker above has been written into
//	                                   the same workspace
//	routing, artifacts, custom-memory.db — NOT workspace ".pi-stack" markers at
//	                                   all: routing.Dir()/taskArtifactRoot()
//	                                   resolve under the HOST's
//	                                   XDG_DATA_HOME/pi-stack tree, never under
//	                                   any workspace's .pi-stack; TestRoutingAnd
...
// workspace's .pi-stack dir can hold today. A future marker must be added
...
func TestMarkerRoundTrip_Profile(t *testing.T) {
	...
	got := readFile(t, filepath.Join(ws, ".pi-stack", "profile"))
	if got != "work\n" {
	...
	if _, err := os.Stat(filepath.Join(ws, ".pi-stack", "profile")); !os.IsNotExist(err) {
...
func TestMarkerRoundTrip_KnowledgeScope(t *testing.T) {
	...
	got := readFile(t, filepath.Join(ws, ".pi-stack", "knowledge.scope"))
	want := "/global/bundle\n/project/bundle\n"
	if got != want {
...
func TestMarkerRoundTrip_OllamaBridgeModel(t *testing.T) {
	...
	got := readFile(t, filepath.Join(ws, ".pi-stack", "ollama-bridge.model"))
	if got != "qwen3.5:9b\n" {
	...
	got2 := readFile(t, filepath.Join(ws2, ".pi-stack", "ollama-bridge.model"))
...
func TestMarkerRoundTrip_HostStateNeverBecomesAWorkspaceFile(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(ws, ".pi-stack", onboardingFileName), []byte(`{"version":1}`), 0o644); err != nil {
		t.Fatal(err)
	}
	env := fakeEnv{present: map[string]bool{}}.env()
	reconcileOnboarding(ws, env, strings.NewReader(""), &out, true, false)
	...
	if _, err := os.Stat(filepath.Join(ws, ".pi-stack", "host-state.json")); !os.IsNotExist(err) {
...
func TestRoutingAndArtifactsAreHostDataRootNotWorkspaceMarkers(t *testing.T) {
	...
	for _, p := range []struct{ name, dir string }{
		...
		if strings.Contains(p.dir, ".pi-stack") {
			t.Errorf("%s dir %q must never contain a .pi-stack path component — it is host data, not a workspace marker", p.name, p.dir)
		}
