...
// runHost implements `pi-stack host` — the UNSANDBOXED escape hatch that execs
// the host-installed `pi` directly (no sbx, no VM). It exists for exactly one
// narrow case: working on pi-stack itself, which needs the host's Docker/sbx/
...
//
//	PI_CODING_AGENT_DIR   = <state>/pi-stack/host-agent  (pi's config dir)
...
func hostGateMessage() string {
	return `pi-stack host is DISABLED (host.enabled = false, the default).

This command runs the agent directly on YOUR machine, and that deletes all
three of pi-stack's safety properties at once:
...
// $XDG_STATE_HOME/pi-stack/host-agent (default ~/.local/state/pi-stack/host-agent).
...
func hostAgentDir() string {
	if x := strings.TrimSpace(os.Getenv("XDG_STATE_HOME")); x != "" {
		return filepath.Join(x, "pi-stack", "host-agent")
	}
	home, err := os.UserHomeDir()
	if err != nil {
		home = "."
	}
	return filepath.Join(home, ".local", "state", "pi-stack", "host-agent")
}
...
pi-stack itself, which needs the host's Docker/sbx/make). Do that work, stay
...
func runHostSetup(errw *os.File) error {
	...
	if err != nil {
		return fmt.Errorf("host setup needs a pi-stack checkout to symlink the harness from: %w", err)
	}
	dir := hostAgentDir()
	...
	link := func(name string, wantDir bool) error {
		...
		if fi, err := os.Lstat(dst); err == nil {
			if fi.Mode()&os.ModeSymlink == 0 {
				fmt.Fprintf(errw, "pi-stack host setup: %s exists and is not a symlink — leaving it alone\n", dst)
				return nil
			}
...
// the services themselves honor (serve.go), so a non-default `pi-stack serve`
...
// hostSelfDevFooter is printed when the workspace is a pi-stack checkout (the
// primary host-mode case): it surfaces the Mode-A/B rule so the edit loop is
// legible without remembering AGENTS.md.
const hostSelfDevFooter = `pi-stack checkout detected: skill/extension/agent edits load live on /reload;
...
func runHostLaunch(o hostOpts) {
	...
	if _, err := os.Stat(filepath.Join(agentDir, "settings.json")); err != nil {
		fmt.Fprintf(os.Stderr, "pi-stack host: %s is not provisioned — run `pi-stack host setup` first\n", agentDir)
		os.Exit(1)
	}
	...
	if _, err := os.Stat(guard); err != nil {
		fmt.Fprintf(os.Stderr, "pi-stack host: guard extension missing (%s).\n", guard)
		fmt.Fprintln(os.Stderr, "Host mode never launches unguarded. Update your pi-stack checkout (it ships")
	...
	if err := os.WriteFile(preamblePath, []byte(hostContextMD), 0o644); err != nil {
		...
		fmt.Fprintf(os.Stderr, "pi-stack host: cannot write host preamble %s: %v\n", preamblePath, err)
		os.Exit(1)
	}
	preamble, err := os.ReadFile(preamblePath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "pi-stack host: cannot read host preamble %s: %v\n", preamblePath, err)
		os.Exit(1)
	}
	...
	if _, err := os.Stat(refs); err == nil {
		if _, err := exec.LookPath("op"); err == nil {
			...
			fmt.Fprintf(os.Stderr, "pi-stack host: %s exists but `op` (1Password CLI) is not on PATH — launching without cloud keys (Ollama-only)\n", refs)
		}
	} else {
