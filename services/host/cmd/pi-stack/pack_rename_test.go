...
func TestPackDir_DefaultsToDefault(t *testing.T) {
	...
	got := config.PackDir()
	want := filepath.Join(dir, "pi-stack", "default")
	if got != want {
...
func TestMigrateLegacyPackDir_PersonalToDefault_RenamesAndUpdatesConfig(t *testing.T) {
	...
	legacy := filepath.Join(data, "pi-stack", "personal")
	...
	want := filepath.Join(data, "pi-stack", "default")
	if root != want {
...
func TestMigrateLegacyPackDir_AlreadyDefault_Idempotent(t *testing.T) {
	...
	newDir := filepath.Join(data, "pi-stack", "default")
...
func TestPackNew_DefaultRoot_ActivatesAndPrintsDefaultLabel(t *testing.T) {
	...
	root := filepath.Join(data, "pi-stack", "default")
	out := buf.String()
...
func migrationTestEnv(t *testing.T) (legacy, def string) {
	...
	t.Setenv("PI_STACK_CONFIG", filepath.Join(cfgDir, "config.toml"))
	t.Setenv("XDG_STATE_HOME", filepath.Join(cfgDir, "state"))
	return filepath.Join(data, "pi-stack", "personal"), filepath.Join(data, "pi-stack", "default")
}
