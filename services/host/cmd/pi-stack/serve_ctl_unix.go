...
// discoverServeProcs finds candidate `pi-stack-host serve` pids when the pidfile
// is gone (e.g. `pi-stack reset` moved the config dir out from under a running
// daemon). It is deliberately LOOSE: `pgrep -f pi-stack-host` returns anything
...
func discoverServeProcs() ([]int, error) {
	out, err := exec.Command("pgrep", "-f", "pi-stack-host").Output()
	if err != nil {
