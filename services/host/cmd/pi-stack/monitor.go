...
const monitorUsage = `usage: pi-stack monitor [name] [--port N] [--bind ADDR]
...
ENV (read by the in-VM extension, documented here for discoverability):
  PI_STACK_MONITOR=0        disable the in-VM tap entirely (no events sent)
  PI_STACK_MONITOR_URL      override the host hub URL
