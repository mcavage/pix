...
// in requiredFlags: pi-stack never authorizes an account without requesting
...
// step entirely); `pi-stack gog setup`/doctor still surface that gap on their
// own. The seam a future `pi-stack setup --account` follow-up (S08) uses to
// decide whether to print the `pi-stack gog setup` hint — it never runs the
// OAuth flow itself.
...
func gogSetup(env shellEnv, opts gogSetupOpts, in io.Reader, out io.Writer, tty bool) error {
	...
	// unsafe route — pi-stack never authorizes without requesting read-only
...
// gateway and pi-stack's persisted config never drift apart:
