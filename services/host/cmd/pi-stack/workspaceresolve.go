...
// created with a CUSTOM name (`pi-stack run --name pi-stack-demo`) breaks the
// old assumption that a workspace's sandbox is always
// `pi-stack-<basename>` (deriveSandboxName). The create receipt
// (sandboxmcpstate.go) now records the canonical Workspace it was created
// for, so verbs that only know a DIR (`pi-stack mcp load NAME [DIR]`, doctor's
// workspace sandbox context) can find the box pi-stack itself created for it.
//
