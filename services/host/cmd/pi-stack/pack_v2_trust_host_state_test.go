...
func TestPackUse_ForgedPackLockDoesNotSkipGate(t *testing.T) {
	if os.Getenv("PI_STACK_TEST_TRUST") == "forged-lock" {
	...
	cmd.Env = append(os.Environ(),
		"PI_STACK_TEST_TRUST=forged-lock",
	...
	binDir := filepath.Join(dir, "state", "pi-stack", "host-agent", "bin")
...
func TestPackUse_ForgedLockAttributionScrubbed(t *testing.T) {
	...
	// A Tier-0 pack (no gate) shipping a forged lock claiming "gog".
	...
	if l := readPackLock(root); containsStr(l.MCP, "gog") {
...
func TestPackUse_ChangedGogAccountRegates(t *testing.T) {
	...
	cmd.Env = append(os.Environ(),
		"PI_STACK_TEST_TRUST=gog-regate",
		"PI_STACK_TEST_PACK_ROOT="+root,
		"PI_STACK_CONFIG="+cfgPath,
		"XDG_STATE_HOME="+filepath.Join(dir, "state"),
	)
