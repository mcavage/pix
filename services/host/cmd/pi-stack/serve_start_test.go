...
func (rec *starterRec) starter() serveStarter {
	return serveStarter{
		hostBin: func() (string, error) { return "/fake/pi-stack-host", nil },
...
func TestEnsureServeSpawnsAndBecomesReady(t *testing.T) {
	...
	out := rec.buf.String()
	if !strings.Contains(out, "starting pi-stack services (memory:11435, knowledge:11436)") {
		t.Errorf("missing starting message, got %q", out)
	}
	if !strings.Contains(out, "pi-stack services ready") {
...
func TestEnsureServeSpawnFailure(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "could not start pi-stack services") {
		t.Fatalf("err = %v, want could-not-start", err)
	}
	if rec.marked {
		t.Error("marker written despite spawn failure")
	}
	if !strings.Contains(rec.buf.String(), "run `pi-stack serve` to see the error") {
...
// PI_STACK_NO_AUTOSERVE set -> no spawn, sentinel error, clear message.
...
func TestEnsureServePidfileOwnedNoDoubleSpawn(t *testing.T) {
	...
	if !strings.Contains(rec.buf.String(), "pi-stack services ready") {
...
func TestEnsureServeColdInitTwoCallersSingleSpawn(t *testing.T) {
	...
	if !strings.Contains(recB.buf.String(), "pi-stack services ready") {
...
func TestEnsureServeRecordPidFailureKillSucceeds(t *testing.T) {
	...
	if strings.Contains(err.Error(), "could not start pi-stack services") {
