...
// localMCPEnv returns a shellEnv whose `pi-stack-host mcp --list` reports the
...
func TestPackUse_SamePackLockForgeryCannotDeleteUserConfig(t *testing.T) {
	...
	if !containsStr(cfg2.MCP, "gog") || !containsStr(cfg2.KnowledgeBundles, userBundleID) {
	...
	if !containsStr(cfg3.MCP, "gog") || !containsStr(cfg3.KnowledgeBundles, userBundleID) {
...
func TestComputeHostBoM_RemoteMCPReferenceIsTier0(t *testing.T) {
	...
	resolver := func() (string, error) { return "pi-stack-host", nil }
	...
	} else if got := strings.Join(b.MCP[0].Argv, " "); got != "pi-stack-host mcp notion" {
