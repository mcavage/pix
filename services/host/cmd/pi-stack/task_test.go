...
func TestTaskPaths_UnderStateHome(t *testing.T) {
	...
	wantCo := filepath.Join(tmp, "pi-stack", "tasks", "abcd1234", "co", "fix-login")
	wantMeta := filepath.Join(tmp, "pi-stack", "tasks", "abcd1234", "meta", "fix-login.json")
	if co != wantCo {
...
func TestRunTask_BareAndHelpPrintUsage(t *testing.T) {
	for _, argv := range [][]string{nil, {"-h"}, {"--help"}} {
		...
		if !strings.Contains(out, "usage: pi-stack task") {
...
func TestGatherTaskState_Clean(t *testing.T) {
	...
	makeTaskClone(t, main, co, "pi-stack/clean", "HEAD")
	env := gitEnv(t, "", nil)
	m := taskMeta{Name: "clean", Sandbox: "pi-stack-t-x-clean", Mainroot: main, Branch: "pi-stack/clean"}
...
func TestGatherTaskState_Dirty(t *testing.T) {
	main := newMainRepo(t)
	co := filepath.Join(t.TempDir(), "co")
	makeTaskClone(t, main, co, "pi-stack/dirty", "HEAD")
	if err := os.WriteFile(filepath.Join(co, "f"), []byte("changed\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	env := gitEnv(t, "", nil)
	m := taskMeta{Name: "dirty", Sandbox: "pi-stack-t-x-dirty", Mainroot: main, Branch: "pi-stack/dirty"}
	st := gatherTaskState(env, m, co)
	if !st.dirty {
...
func TestGatherTaskState_CommittedUnpushedNoRemote(t *testing.T) {
	...
	env := gitEnv(t, "", nil)
	m := taskMeta{Name: "work", Sandbox: "pi-stack-t-x-work", Mainroot: main, Branch: "pi-stack/work"}
	st := gatherTaskState(env, m, co)
	if st.unrec != 1 {
...
func TestGatherTaskState_CommittedButInMain(t *testing.T) {
	...
	co := filepath.Join(t.TempDir(), "co")
	makeTaskClone(t, main, co, "pi-stack/inmain", "HEAD")
	env := gitEnv(t, "", nil)
	m := taskMeta{Name: "inmain", Sandbox: "pi-stack-t-x-inmain", Mainroot: main, Branch: "pi-stack/inmain"}
	st := gatherTaskState(env, m, co)
	if st.unrec != 0 {
...
func TestGatherTaskState_ScratchBranchCommit(t *testing.T) {
	...
	taskTip := strings.TrimSpace(tgit(t, co, "rev-parse", "pi-stack/work"))
	...
	env := gitEnv(t, "", nil)
	m := taskMeta{Name: "work", Sandbox: "x", Mainroot: main, Branch: "pi-stack/work"}
	st := gatherTaskState(env, m, co)
	if st.unrec != 1 {
...
func TestExecuteTaskTeardown_SnapshotCapturesScratchCommit(t *testing.T) {
	...
	sbxname := "pi-stack-t-x-work"
	env := gitEnv(t, "", nil) // sbx reads absent; `sbx rm -f` succeeds.
	m := taskMeta{Name: "work", Sandbox: sbxname, Mainroot: main, Branch: "pi-stack/work"}
	st := gatherTaskState(env, m, co)

	var out bytes.Buffer
	recovered := "refs/pi-stack/recovered/work"
...
func TestTaskMeta_TamperedBranchNeverTargetsMainRef(t *testing.T) {
	...
	// Every fetch destination refspec must live under refs/pi-stack/{_chk,recovered}/work.
	for _, f := range fetches {
		...
		ok := strings.HasPrefix(dst, "refs/pi-stack/_chk/work") || strings.HasPrefix(dst, "refs/pi-stack/recovered/work")
		if !ok {
			t.Errorf("fetch targeted a ref outside refs/pi-stack/{_chk,recovered}/work: %q", dst)
		}
	}
}
...
func TestGatherTaskState_ProbeFailIsUnknown(t *testing.T) {
	...
	badmain := t.TempDir()
	env := gitEnv(t, "", nil)
	m := taskMeta{Name: "work", Sandbox: "x", Mainroot: badmain, Branch: "pi-stack/work"}
	st := gatherTaskState(env, m, co)
	if !st.unknown {
...
func TestTaskNew_SourceOnlyRefChecksOutByOID(t *testing.T) {
	...
	if branch != "pi-stack/remote-oid" {
		t.Errorf("clone branch = %q, want pi-stack/remote-oid", branch)
	}
...
func TestExecuteTaskTeardown_FreshRunningRefuses(t *testing.T) {
	...
	var out bytes.Buffer
	rc := executeTaskTeardown(env, &out, m, co, "work", "refs/pi-stack/recovered/work", false, st)
	if rc != 1 {
...
func TestExecuteTaskTeardown_ForceOverridesRunning(t *testing.T) {
	...
	var out bytes.Buffer
	rc := executeTaskTeardown(env, &out, m, co, "work", "refs/pi-stack/recovered/work", true, st)
	if rc != 0 {
...
func TestTaskNew_ExistingCheckoutNotDeleted(t *testing.T) {
	...
	if os.Getenv("PI_STACK_TASKNEW_EXIST") == "1" {
		if err := os.Chdir(os.Getenv("PI_STACK_TASKNEW_MAIN")); err != nil {
			t.Fatal(err)
		}
	...
	cmd.Env = append(os.Environ(),
		"PI_STACK_TASKNEW_EXIST=1",
		"PI_STACK_TASKNEW_MAIN="+main,
...
func TestTaskNew_OwnedRollbackOnLaunchFailure(t *testing.T) {
	...
	if os.Getenv("PI_STACK_TASKNEW_BOOM") == "1" {
		if err := os.Chdir(os.Getenv("PI_STACK_TASKNEW_MAIN")); err != nil {
			t.Fatal(err)
		}
...
func TestTaskRm_AcquiresLock(t *testing.T) {
	if os.Getenv("PI_STACK_TASKRM_LOCK") == "1" {
		if err := os.Chdir(os.Getenv("PI_STACK_TASKRM_MAIN")); err != nil {
			t.Fatal(err)
		}
	...
	cmd.Env = append(os.Environ(),
		"PI_STACK_TASKRM_LOCK=1",
		"PI_STACK_TASKRM_MAIN="+main,
		"XDG_STATE_HOME="+state,
		"PI_STACK_CONFIG="+filepath.Join(t.TempDir(), "config.toml"),
		"PI_STACK_PROFILE=")
	out, err := cmd.CombinedOutput()
	var ee *exec.ExitError
	if !errors.As(err, &ee) {
