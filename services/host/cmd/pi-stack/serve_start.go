...
// (docs/design/serve-lifecycle.md §1): the first `pi-stack run` / `pi-stack
// memory …` / `pi-stack knowledge query` spins up a detached `pi-stack-host
...
const autoserveEnvVar = "PI_STACK_NO_AUTOSERVE"
...
// ensureServeRunTimeout is the SHORTER budget `pi-stack run` uses: a sandbox
...
type serveStarter struct {
	...
	// its own deadline, so a wedged lock-holder can never hang `pi-stack run`
...
func ensureServe(st serveStarter, cfg *config.Config, opts ensureServeOpts) error {
	...
	criticalSection := func() error {
		...
		// Idempotency via pidfile: a live, verified-ours `pi-stack-host serve`
		...
		bin, err := st.hostBin()
		if err != nil {
			return fmt.Errorf("could not start pi-stack services: %v. run `pi-stack serve` to see the error", err)
		}
		fmt.Fprintf(st.stderr, "starting pi-stack services (%s)…\n", describeServePorts(ports))
	...
	for {
		...
		if !st.now().Before(deadline) {
			msg := fmt.Sprintf("pi-stack services did not become ready in %s (another process holds the start lock %s)",
	...
	for {
		if allUp() {
			if (spawned || waiting) && !opts.Quiet {
				fmt.Fprintln(st.stderr, "pi-stack services ready")
			}
			return nil
		}
		if !st.now().Before(deadline) {
			logPath := st.logPath()
			msg := fmt.Sprintf("pi-stack services did not become ready in %s", timeout)
...
// for a live, VERIFIED-ours `pi-stack-host serve` — the same refuse-unless-sure
