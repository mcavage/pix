...
// status <name>`), and FAIL with the exact repair commands (`pi-stack mcp
// bundle`, `pi-stack mcp auth <name>`) on any gap. The gate never opens an
...
func catalogMCPState(env shellEnv, mcpOut string, mcpOK bool, name string) catalogMCPReadiness {
	...
	// starts the OAuth flow (that is `pi-stack mcp auth <name>`, run by the
...
func verifyCatalogMCPReady(env shellEnv, names []string) error {
	...
	for _, n := range catalog {
		...
		case catalogMCPUnregistered:
			return fmt.Errorf("catalog MCP server %q is not registered with the sbx gateway — nothing was saved; register the shipped catalog (`pi-stack mcp bundle`), authorize it (`pi-stack mcp auth %s`), then re-run", n, n)
		case catalogMCPUnauthorized:
			return fmt.Errorf("catalog MCP server %q is registered but not authorized — nothing was saved; run `pi-stack mcp auth %s`, then re-run", n, n)
		...
		default: // catalogMCPUnverifiable
			return fmt.Errorf("could not verify catalog MCP server %q is registered and authorized (sbx probe unavailable, failed, or timed out) — nothing was saved; fix sbx and retry, or prepare it first: `pi-stack mcp bundle`, then `pi-stack mcp auth %s`", n, n)
		}
	}
	return nil
}
