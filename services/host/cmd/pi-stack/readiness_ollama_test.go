...
func TestOllamaSandboxAxisNeverCreatesASandbox(t *testing.T) {
	...
	if !strings.Contains(c[0].evidence, "pi-stack run") {
...
func TestBindInferenceNeverProducesReady(t *testing.T) {
	...
	env := ollamaEnv(t, "", true, "")
	no := false
	s := buildSnapshot(Request{Axes: []Axis{axisOllamaSandbox}}, ollamaReadinessAxes(cfg, env, "pi-stack-demo", &no))
	c, _ := s.Checks(axisOllamaSandbox)
	...
	yes := true
	s2 := buildSnapshot(Request{Axes: []Axis{axisOllamaSandbox}}, ollamaReadinessAxes(cfg, env, "pi-stack-demo", &yes))
