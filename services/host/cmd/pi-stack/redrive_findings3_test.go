...
func TestStatusMCPRowsEmptyCfgReceiptPreloaded(t *testing.T) {
	...
	if err := writeCreateReceipt(stateDir, "pi-stack-proj", "", []string{"notion"}, receiptClock); err != nil {
		t.Fatal(err)
	}
	...
	r := st.MCPRows[0]
	if r.Name != "notion" || r.State != mcpJoinPreloaded || r.Sandbox != "pi-stack-proj" {
		t.Errorf("row = %+v, want preloaded notion on pi-stack-proj", r)
	}
...
func TestStatusMCPRowsEmptyCfgNoSandboxes(t *testing.T) {
	...
	st := gatherStatus(cfg, "default", env)
	if len(st.MCPRows) != 0 {
		t.Errorf("MCPRows = %+v, want none (nothing configured, no pi-stack sandboxes)", st.MCPRows)
	}
	if len(st.MCPServers) != 0 {
