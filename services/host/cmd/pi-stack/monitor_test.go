...
func TestMonitorHelp(t *testing.T) {
	...
	for _, argv := range [][]string{{"-h"}, {"--help"}, {"--help", "mybox"}} {
		...
		if !strings.Contains(out.String(), "usage: pi-stack monitor") {
...
func TestMonitorUsage(t *testing.T) {
	...
	if !strings.HasPrefix(u, "usage: pi-stack monitor") {
		t.Errorf("verbUsage(monitor) = %q, want prefix %q", u, "usage: pi-stack monitor")
	}
}
