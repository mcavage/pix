...
// it: "pi-stack-t-" (11 chars) becomes "pix-t-" (6 chars) in U-W3.09. Five
// more characters of budget then survive maxSandboxNameLen (63) truncation,
// which moves EVERY threshold this file pins.
//
...
func TestBoundSandboxName_ExactFitBoundary(t *testing.T) {
	...
	// Fixed overhead = len("pi-stack-t-") + "-" + repokey + "-" = 11+1+8+1 = 21.
	// Budget left for label+name = 63-21 = 42; with label=12, name budget = 30.
	...
	// name length 30: compose is EXACTLY 63 -- fits with zero trimming.
	name30 := strings.Repeat("a", 30)
	want30 := "pi-stack-t-repolabel12x-abcd1234-" + name30
	if got := boundSandboxName(label, repokey, name30, ""); got != want30 {
		t.Errorf("name len 30 (exact fit): got %q, want %q", got, want30)
	}
	if len(want30) != 63 {
		t.Fatalf("test fixture arithmetic is wrong: want30 is %d chars, want 63", len(want30))
	}

	// name length 31: ONE character over -- this is the pinned threshold.
	// boundSandboxName must now hash-tag-trim the name (never the label or
	// the repokey) down to fit. The exact trimmed+hashed string is pinned
	// verbatim below; it was captured by calling boundSandboxName directly,
	// not computed by hand.
	name31 := strings.Repeat("a", 31)
	want31 := "pi-stack-t-repolabel12x-abcd1234-aaaaaaaaaaaaaaaaaaa-61c60b487d"
	if got := boundSandboxName(label, repokey, name31, ""); got != want31 {
		t.Errorf("name len 31 (one over the exact-fit boundary): got %q, want %q", got, want31)
	}
	if len(want31) != 63 {
		t.Errorf("trimmed composite is %d chars, want exactly maxSandboxNameLen (63)", len(want31))
	}
}
...
func TestBoundSandboxName_NameOverflowStaysAtTheSameTrimWindow(t *testing.T) {
	...
	want := "pi-stack-t-repolabel12x-abcd1234-aaaaaaaaaaaaaaaaaaa-11ee391211"
...
func TestBoundSandboxName_LabelUntouchedWhenNameIsSmall(t *testing.T) {
	...
	want := "pi-stack-t-reallylonglabelthatoverflows12345-abcd1234-n"
	if got := boundSandboxName(longLabel, "abcd1234", "n", ""); got != want {
		t.Errorf("got %q, want %q", got, want)
	}
	if len(want) != 55 {
		t.Errorf("composite is %d chars, want 55 (well under the 63 cap -- no trim expected)", len(want))
	}
}
...
func TestBoundSandboxName_NameHitsFloorThenLabelTrims(t *testing.T) {
	...
	name60 := strings.Repeat("a", 60)
	want := "pi-stack-t-reallylonglabelthatoverflows12-abcd1234-a-11ee391211"
	...
	// The name-at-floor segment is exactly 12 chars ("a" + "-" + 10 hex),
	// and it is the SAME hash as the name-len-60 case above -- hashTagTrim
	// depends only on the string being hashed, never on the label around it.
	if !strings.HasSuffix(got, "-a-11ee391211") {
...
func TestBoundSandboxName_LabelDroppedToRepoThenProfileTrims(t *testing.T) {
	...
	want := "pi-stack-t-repo-abcd1234-n-aaaaaaaaaaaaaaaaaaaaaaaaa-11ee391211"
