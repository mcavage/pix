...
const stateUsage = `usage: pi-stack state <backup|restore|reset|uninstall> [args]
...
Run ` + "`pi-stack help <subcommand>`" + ` for full flags.
`
...
func runState(argv []string) {
	...
	case "uninstall":
		runUninstall(argv[1:])
	default:
		fmt.Fprintf(os.Stderr, "pi-stack state: unknown subcommand %q\n\n%s", argv[0], stateUsage)
		os.Exit(2)
	}
}
