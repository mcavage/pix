// gworkspace_test.go covers the PUBLIC façade in gworkspace.go: `pi-stack
...
func TestParseGworkspaceSetupArgs(t *testing.T) {
	...
	if _, err := parseGworkspaceSetupArgs([]string{"-h"}); err != errHelpRequested {
...
func TestRunGworkspaceCmd_UnknownSubcommandExitsNonZero(t *testing.T) {
	if os.Getenv("PI_STACK_TEST_GWORKSPACE_DISPATCH") == "unknown" {
		runGworkspaceCmd([]string{"bogus"})
		return
	}
	cmd := exec.Command(os.Args[0], "-test.run", "^TestRunGworkspaceCmd_UnknownSubcommandExitsNonZero$")
	cmd.Env = append(os.Environ(), "PI_STACK_TEST_GWORKSPACE_DISPATCH=unknown")
...
func TestRunGworkspaceCmd_NoArgsExitsNonZero(t *testing.T) {
	if os.Getenv("PI_STACK_TEST_GWORKSPACE_DISPATCH") == "noargs" {
		runGworkspaceCmd(nil)
		return
	}
	cmd := exec.Command(os.Args[0], "-test.run", "^TestRunGworkspaceCmd_NoArgsExitsNonZero$")
	cmd.Env = append(os.Environ(), "PI_STACK_TEST_GWORKSPACE_DISPATCH=noargs")
	out, err := cmd.CombinedOutput()
	if err == nil {
		t.Fatalf("no subcommand must exit non-zero, output:\n%s", out)
	}
	if !strings.Contains(string(out), "usage: pi-stack gworkspace") {
...
func TestGworkspaceStatus_NotConfigured(t *testing.T) {
	...
	if !strings.Contains(out.String(), "not configured — set it up: pi-stack gworkspace setup") {
		t.Errorf("expected the not-configured line, got:\n%s", out.String())
	}
}

// TestGworkspaceStatus_ConfiguredNoAccount: gwServerName is in the MCP set but
// no account is authorized -> a TODO pointing at `pi-stack gworkspace setup`
...
func TestGworkspaceStatus_ConfiguredNoAccount(t *testing.T) {
	...
	if !strings.Contains(out.String(), "no account is authorized") || !strings.Contains(out.String(), "pi-stack gworkspace setup") {
...
func TestGworkspaceDisable_PresentRemovesRegistrationAndConfig(t *testing.T) {
	...
	if cfg.GogAccount != "" || containsStr(cfg.MCP, gwServerName) {
...
// never reference a `pi-stack gog ...` command — that verb tree is DELETED,
...
func TestGworkspaceDisable_NeverPrintsLegacyGogVerb(t *testing.T) {
	...
	if strings.Contains(out.String(), "pi-stack gog ") {
		t.Errorf("disable output references the deleted `pi-stack gog` verb tree, got:\n%s", out.String())
	}
}
