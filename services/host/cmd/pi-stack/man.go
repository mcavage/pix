...
// manPage is the authored roff man page, embedded at build time so `pi-stack
...
const manUsage = `usage: pi-stack man

Render the embedded pi-stack manual page. The page is baked into the binary, so
...
the page is written straight to stdout, so 'pi-stack man | grep foo' works.
...
func renderMan(env manEnv) {
	...
	if tty {
		...
		if pg := resolvePager(env); pg != "" {
			if err := env.run(env, pg, nil, "", env.stdout); err == nil {
				fmt.Fprintln(env.stderr, "pi-stack man: no man renderer (mandoc/man/groff) found; showing raw roff")
				return
			}
		}
		fmt.Fprintln(env.stderr, "pi-stack man: no man renderer found; showing raw roff")
	...
	// Last resort: raw roff. Still succeeds so `pi-stack man | grep foo` works.
	_, _ = env.stdout.Write(manPage)
}
