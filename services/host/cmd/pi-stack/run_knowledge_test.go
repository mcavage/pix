...
// <dir>/.pi-stack/knowledge containing the resolved absolute bundle path, and
...
func TestKnowledgeUseProject_LocalPath(t *testing.T) {
	...
	pointer := filepath.Join(repo, ".pi-stack", "knowledge")
...
func TestWireKnowledgeScope_NoBundles(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(ws, ".pi-stack", "knowledge.scope")); !os.IsNotExist(err) {
...
func scopeLines(t *testing.T, workspace string) []string {
	t.Helper()
	b := readFile(t, filepath.Join(workspace, ".pi-stack", "knowledge.scope"))
