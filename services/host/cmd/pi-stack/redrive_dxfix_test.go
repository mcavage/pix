...
//
//  1: `pi-stack route --help` pointed a consumer at the embedded repo source
//     (services/host/routing/defaults/scorecard.json) for the override file
//     they should hand-edit. That path only exists inside a pi-stack repo
//     checkout and means nothing on a consumer machine — the consumer-facing
//     recovery path must be the live override dir
//     (~/.local/share/pi-stack/routing/, honoring $ROUTING_DIR/$XDG_DATA_HOME);
//     a mention of the maintainer's embedded repo source is fine ONLY when
//     explicitly labeled as such.
//  3: `pi-stack mcp register/load/auth/bundle` (and, by the same policy,
...
//     `pi-stack memory`/`secret` already use, while still printing the exact
...
//     `pi-stack agent new`'s next-steps, `pi-stack agent reassess --model`'s
//     guidance, and the removed `pi-stack evals` message.
...
// mention of the repo's embedded default source (for a pi-stack maintainer
...
func TestAgentReassessModel_PointsAtLiveScorecardPath(t *testing.T) {
	if os.Getenv("PI_STACK_DXFIX_REASSESS") == "1" {
...
// (registerServers never writes config — only `pi-stack config set` may).
func TestRunMcpRegister_AbsentSbxExitsServiceDownNoConfigMutation(t *testing.T) {
	...
	// A fake pi-stack-host next to nothing real: `mcp --list` reports slack as
	...
	cmd.Env = append(envWithout(os.Environ(), "PATH"),
		"PI_STACK_DXFIX_MCP_REGISTER=1",
		"PI_STACK_CONFIG="+cfgPath,
		"PATH="+binDir, // pi-stack-host resolvable; sbx is not
	)
	var out bytes.Buffer
	cmd.Stdout = &out
	cmd.Stderr = &out
