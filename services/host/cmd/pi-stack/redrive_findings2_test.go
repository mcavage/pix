...
//  13: `pi-stack mcp load` validates NAME [DIR] strictly before deriving
//      anything.
...
func TestVerifyCatalogMCPReady_UnregisteredNamesBundleAndAuth(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "pi-stack mcp bundle") || !strings.Contains(err.Error(), "pi-stack mcp auth notion") {
...
func TestVerifyCatalogMCPReady_UnauthorizedNamesAuthCommand(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "pi-stack mcp auth notion") {
...
func TestVerifyCatalogMCPReady_ExplicitPolicyDenial(t *testing.T) {
	...
	if strings.Contains(err.Error(), "pi-stack mcp auth notion`") && strings.Contains(err.Error(), "then re-run") {
...
// invariant end to end: `pi-stack setup --mcp notion` with notion unregistered
...
func TestSetupHostPhase_CatalogGate_NoSaveNoKeysOnGap(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "pi-stack mcp bundle") {
...
func TestReconcileOnboarding_CatalogGateLeavesFileAndConfig(t *testing.T) {
	...
	if !strings.Contains(out.String(), "pi-stack mcp bundle") {
...
func TestMcpLocalCheck_PolicyDeniedVerdict(t *testing.T) {
	const hostBin = "/usr/local/bin/pi-stack-host"
	env := shellEnv{
		lookPath: func(name string) (string, error) {
			if name == "sbx" {
				return "/usr/bin/sbx", nil
...
func TestGogRegistrationCheck_TriState(t *testing.T) {
	...
	if c.result() != verdictTodo || c.todo != "pi-stack mcp register" {
