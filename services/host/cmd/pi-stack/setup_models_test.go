// setup_models_test.go — S08: `pi-stack setup` local-model readiness + pulls.
//
...
func TestSetup_RetiredKeysDroppedWithOneNotice(t *testing.T) {
	...
	env := modelsSetupEnv(t, w)
	stubProvisionKeysOK(t)
	cfgPath := os.Getenv("PI_STACK_CONFIG")
...
func TestSetupModels_ReceiptWritten(t *testing.T) {
	...
	path := filepath.Join(os.Getenv("XDG_STATE_HOME"), "pi-stack", "setup", "models.json")
