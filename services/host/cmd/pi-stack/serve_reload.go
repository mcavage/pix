...
// §3): after `pi-stack config set/unset` writes a DAEMON-AFFECTING key, the
// running `pi-stack-host serve` is restarted per its lifecycle mode so the
...
// (it runs the same `pi-stack-host serve`), so the pidfile alone cannot tell
...
func propagateServeConfig(rl serveReloader, out io.Writer) {
	...
	case serveManaged:
		if err := rl.kickManaged(); err != nil {
			fmt.Fprintf(out, "warning: could not restart the managed pi-stack service (%v) — restart it manually to apply the change.\n", err)
			return
		}
		fmt.Fprintln(out, "restarted managed pi-stack services to apply the change.")
	case serveLazy:
		stopped, err := rl.stopServe(io.Discard)
		if err != nil {
			fmt.Fprintf(out, "warning: could not stop the background pi-stack services (%v) — restart them manually to apply the change.\n", err)
			return
		}
		if !stopped {
			...
			fmt.Fprintln(out, "warning: the background pi-stack services were not stopped — run `pi-stack serve stop` then restart to apply the change.")
			return
		}
		if err := rl.ensure(); err != nil {
			fmt.Fprintf(out, "warning: pi-stack services were stopped but did not restart (%v) — run `pi-stack serve` or retry.\n", err)
			return
		}
		fmt.Fprintln(out, "restarted pi-stack services (background) to apply the change.")
	case serveForeground:
		// Never kill a process the user is watching in their own terminal.
		fmt.Fprintln(out, "note: a foreground `pi-stack serve` is running — restart it (Ctrl-C, re-run) to apply this change.")
	default: // serveDown
		fmt.Fprintln(out, "note: the change applies next time pi-stack services start.")
	}
}
