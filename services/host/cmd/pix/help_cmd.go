...
// <anything>` is a question, never a mistake, so it answers with the tiered screen
// instead of a parse error (`pix help --nope` wants help, and gets it, exit 0).
package main
...
// Run: every verb is typed, so `pix help ls` re-enters the root as `pix ls --help`
// and the usage a user reads is generated by what parses it.
func (c *helpCmd) Run(d *cli.Deps) error {
	if testSeams.legacy != nil {
		testSeams.legacy("help", c.Args)
		return nil
	}
	if len(c.Args) > 0 {
		if c.Args[0] == "--all" {
