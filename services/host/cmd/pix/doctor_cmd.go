// doctor_cmd.go — `pix doctor` and `pix status` as typed root children. They
// own an exit contract and nothing else: probing and rendering are
// workflow/doctor's, and a cli.SilentError carries doctor's own code, because
// its rendered table already said everything there is to say.
package main
...
// made does NOT fail the process), 1 when one did or the config failed to
// load, 2 for a usage error.
...
func (c *doctorCmd) Run(d *cli.Deps) error {
	...
	if err != nil {
		// Doctor DOES fail here: an unreadable config is a verified gap in
		// something required. Status renders the same fact and exits 0.
...
//
// It exits 0 whatever the verdict (doctor.StatusExit): the landing screen runs
// under `set -e` and in prompts, so a probe that could not see something must
// not take a shell down with it. The verdict is one `pix doctor` (or --json's
// `exit` field) away.
...
// the workspace the MCP attachment answer is about. An unresolvable workspace
// stays empty, reported as "attachment unknown" rather than guessed.
