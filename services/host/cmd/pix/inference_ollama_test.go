...
func ollamaListEnv(tags []string, _ string, totalGB float64) shellEnv {
	env := shellEnv{System: &systest.Fake{LookPathFn: func(string) (string, error) { return "/usr/local/bin/ollama", nil }}}
...
func TestNilOllamaProbeSeamLeavesBindingsUnverified(t *testing.T) {
	cfg := ollamaCfgWith(binding("ollama/qwen3.5:9b"))
	probe, err := verifyOllamaInference(cfg, shellEnv{System: &systest.Fake{}}, io.Discard)
	if err == nil {
...
func TestVerifyOllamaInferenceUsesResolvedEndpoint(t *testing.T) {
	cfg := ollamaCfgWith(binding("ollama/qwen3.5:9b"))
	var seen string
	env := shellEnv{System: &systest.Fake{GetenvFn: func(name string) string {
		if name == "OLLAMA_HOST" {
