...
// composition pack deliberately does not do for itself: the real env, the MCP
// register function, the local-MCP classifier, and the exit codes.
//
// The typed tree replaced a hand-written Usage constant documenting flags no
// code path accepted: generated help cannot describe a flag that does not
// exist. What is NOT here is pack's trust and service admission — `use`/`add
// mcp` still cross the same Tier-1 gate, fingerprint and acceptance store.
...
// packAddArgs renders the typed fields into the argv pack still takes. pack's
// own parse is load-bearing for the trust tests that drive RunPackAdd/
// RunPackUse directly; retiring it is a capability-side change (U11e).
