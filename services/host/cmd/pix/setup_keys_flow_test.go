...
func stepEnv(t *testing.T, refsContent, sbxLsOut string, opReadVal string) (shellEnv, *[]string) {
	...
	files := map[string]string{"/cfg/pix/op-refs.env": refsContent}
	calls := &[]string{}
...
func TestSetupProvisionKeys_OpNotInstalled_FailsWithExactFix(t *testing.T) {
	...
	if !strings.Contains(out.String(), "1Password provider setup requires") || !strings.Contains(out.String(), "op` CLI, but it isn't installed") {
		t.Errorf("must explain op is missing (mode-scoped copy), got:\n%s", out.String())
	}
...
// `pix secret set` command is printed per missing provider, and setup
// fails.
func TestSetupProvisionKeys_MissingRefs_NonInteractive_ExactCommandsNoPrompt(t *testing.T) {
	...
	for _, want := range []string{
		"pix secret set ANTHROPIC_API_KEY op://Vault/Item/field",
		"pix secret set OPENAI_API_KEY op://Vault/Item/field",
		"pix secret set GEMINI_API_KEY op://Vault/Item/field",
	} {
...
func TestSetupProvisionKeys_FinalProbe_SbxCommandFails_FailsClosed(t *testing.T) {
	...
	fakeOf(env).RunFn = func(name string, args ...string) (string, error) {
		...
		case name == "sbx" && len(args) >= 2 && args[0] == "secret" && args[1] == "ls":
			calls++
			// The first call (reconcile's own probe) succeeds; only the LAST call
			// (the FINAL probe) fails.
			if calls <= 1 {
				return "anthropic\nopenai\ngoogle\n", nil
			}
