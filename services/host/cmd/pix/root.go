...
// task new/ls/path/rm, env list/add/upgrade/show/default/trust, secret
...
type rootCmd struct {
	...
	Env    envCmd    `cmd:"" group:"Environments & credentials" help:"Named environments under ~/.pix/envs: list | add | upgrade | show | default | trust."`
...
func isEnvKnownVerb(v string) bool {
	switch v {
	case "list", "add", "upgrade", "show", "default", "trust":
		return true
	}
	return false
}
