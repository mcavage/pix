...
// TestStatusHostGlobalNoAttachmentClaim: with no pix sandbox discovered,
...
func TestStatusMCPRowsJSONGolden(t *testing.T) {
	...
	golden := `[
  {
    ...
    "sandbox": "pix-proj",
    "state": "preloaded",
    "evidence": "preloaded by pix at create"
  },
  {
    "name": "slack",
    "registered": "yes",
  ...
  },
  {
    ...
    "evidence": "no receipt entry; attach live with ` + "`pix mcp load notion`" + ` or recreate with ` + "`pix run --replace`" + `"
  },
  {
    "name": "linear",
    "registered": "no",
