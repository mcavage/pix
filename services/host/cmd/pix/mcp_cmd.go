...
// mcp_cmd.go is `pix mcp` under the cli command contract: the typed verb tree,
// plus the composition the mcp capability deliberately does not do for itself
// (the active pack's containers, a real env, the host-binary resolver).
//
// What went away with the switch: a Usage constant that listed five
// subcommands beside the switch that dispatched them and a `load` arity parser
// that re-derived "takes no flags" by hand, three copies of "print the error,
// pick an exit code, os.Exit", and the bare-verb usage dump. kong owns the
// grammar; mcpFailed owns the one exit mapping.
//
// The honesty contract is unchanged and is the reason this verb has a custom
// mapping at all: a subcommand that PROMISES an operation must never exit 0
// having done nothing. sbx missing prints the exact command to run by hand and
// exits 3 (rpc.ExitServiceDown); a child's own exit code is propagated as-is.
...
// in the verb's own words (as the seams did) and returns a SilentError so the
// root does not report the same cause twice.
...
// mcpRegisterCmd registers the requested local stdio servers (or, with no
// names, the local ones in cfg.MCP) with the sbx gateway — `make mcp-register`
// without the repo or the Makefile.
...
// forward verbatim, and the plain-text attachment note is skipped whenever
// they are present, so a script parsing structured output never has to filter
// out prose.
...
func (c *mcpLoadCmd) Run(d *cli.Deps) error {
	// Arity and flag-shape are kong's now; what survives is the VALIDATION a
	// parser cannot do — a blank name, and a workspace `pix run` would refuse.
	...
	// sandbox created with a CUSTOM name (`pix run --name pix-demo`) is loaded
	// into, not a same-named stranger; an ambiguous or untrustworthy mapping
	// REFUSES rather than targeting an arbitrary box.
	...
	if _, err := exec.LookPath("sbx"); err != nil {
		...
		// done nothing — see mcp.ErrSbxUnavailable. McpWouldRun preserves the
	...
	if errors.As(err, &recErr) {
		// The attach itself succeeded — only the local receipt failed. Report
		// that distinctly (never a plain success) and exit non-zero so
		// doctor/status degrade honestly instead of trusting a record that was
		// never written.
...
// mcpLoadArgs rebuilds the pair ParseMcpLoadArgs validates. That validator is
// shared with the receipt tests and states the load contract in one place; the
// only thing kong took from it is counting.
...
// mcpAuthCmd is a thin passthrough to `sbx mcp auth <args...>` — the native
// hosted-control-plane OAuth flow for remote servers, so repo-less hosts get
...
// mcpBundleCmd manages the shipped public MCP catalog bundle
// (notion/atlassian/granola). Bare (or `add`) registers the pinned pix catalog
// in one step — the remote set that matches this build. `ls`/`rm` (and any
// other args) forward verbatim to `sbx mcp bundle`.
