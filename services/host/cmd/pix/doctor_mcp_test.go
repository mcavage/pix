...
// mcpFake builds a fakeEnv with the canonical pix-host resolved and slack
...
func TestMCPAttachmentFromReceipt(t *testing.T) {
	...
	t.Run("complete receipt, no entry -> verified registered-not-attached TODO", func(t *testing.T) {
		...
		// the exact `pix mcp load NAME <workspace>` command, consistent
...
func TestMCPHostGlobalContext(t *testing.T) {
	...
	t.Run("sandbox positively absent -> not-created-yet note, no attachment", func(t *testing.T) {
		f := mcpFake()
		f.output["sbx ls"] = "pix-other  running  /elsewhere\n"
...
func TestMCPCanonicalExecutableGate(t *testing.T) {
	...
	t.Run("look-alike pix-host path is never exec'd", func(t *testing.T) {
		f := mcpFake()
	...
	t.Run("registered spelling is normalized to the resolver token (no TOCTOU)", func(t *testing.T) {
		...
		reg := "/usr/local/bin/../bin/pix-host mcp slack"
		f := mcpFake()
		f.output["sbx mcp get slack"] = "name: slack\ncommand: " + reg + "\n"
		var execd string
		env := f.env()
	...
	t.Run("op-wrapped canonical spawn execs the canonical op token", func(t *testing.T) {
		...
		// op-refs path (PIX_CONFIG's dir) — anything else is rejected.
		f.envVars = map[string]string{"PIX_CONFIG": "/x/config.toml"}
		f.statFile = map[string]bool{"/x/op-refs.env": true}
		reg := "/usr/bin/op run --no-masking --env-file=/x/op-refs.env -- /usr/local/bin/pix-host mcp slack"
		f.output["sbx mcp get slack"] = "name: slack\ncommand: " + reg + "\n"
		var execd string
		env := f.env()
		inner := systest.Of(env.System).RunFn
