// monitor_tui.go — Unit B of `pix monitor` (see
...
// maxRows bounds retained rows so a long-running `pix monitor` session
...
// piInvocationRe matches a `pi` or `pix` COMMAND TOKEN — anchored at
...
var piInvocationRe = regexp.MustCompile(`(?i)(^|[\s;&|(])(pi|pix)([\s"']|$)`)

// toolInvokesPi reports whether a tool's ArgsSummary actually runs `pi` or
// `pix` as a command (as opposed to merely mentioning something that
...
func (m Model) headerLine() string {
	...
	if len(m.sandboxOrder) == 1 {
		...
		return fmt.Sprintf("pix monitor  %s  events=%d", sandboxTabName(m.sandboxOrder[0]), len(m.rows))
	}
	sandbox := m.cfg.Filter
	if sandbox == "" {
		sandbox = "all"
	}
	return fmt.Sprintf("pix monitor  sandbox=%s  events=%d", sandbox, len(m.rows))
}
...
func (m Model) focusHeaderLine() string {
	...
	return fmt.Sprintf("pix monitor  focus %s  events=%d", label, m.sessionRowCount[m.focusSession])
}
