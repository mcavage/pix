...
// Every later verb unit (E1.10 `add`, E1.12 `edit`) adds its own ONE field
// line here and lands in ID order (units.json's file-conflict table: "E1.9
// owns the struct; each verb unit adds one field line + its own file. Land
// in ID order, rebase."). E1.11 (this unit) added `use`, `forget`, and the
// `rm` pointer error below.
//
...
unregisters, and it deletes nothing. 'add'/'edit' land in later units;
ls, show, use, review and forget work now.
