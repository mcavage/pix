...
const memoryUsage = `usage: pix memory <recall|remember|forget|learnings|stats> [args]
...
func memoryRemember(argv []string, c rpcClient, out io.Writer, profile string) error {
	...
	if content == "" {
		return usageErr("usage: pix memory remember <text...>")
	}
...
func memoryForget(argv []string, c rpcClient, out io.Writer, profile string) error {
	...
	if len(positional) != 1 {
		return usageErr("usage: pix memory forget <id> [--json]")
	}
...
func memoryLearnings(argv []string, c rpcClient, out io.Writer, profile string) error {
	...
	if len(positional) > 0 {
		return usageErr("usage: pix memory learnings [--min N] [--json]")
	}
...
func memoryStats(argv []string, c rpcClient, out io.Writer, profile string) error {
	...
	if len(positional) > 0 {
		return usageErr("usage: pix memory stats [--json]")
	}
