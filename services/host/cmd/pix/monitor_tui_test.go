...
func TestMonitorTUI_HelpOverlayStartsAtTopAndScrolls(t *testing.T) {
	...
	m = key(t, m, runeKey("?"))
	view := m.View()
	if !strings.Contains(view, "pix monitor — keys") {
...
func TestMonitorTUI_TreeGroupsInterleavedSessionsContiguously(t *testing.T) {
	m := NewModel(TUIConfig{})
	m = feed(t, m, mkSandboxCtx("pix-dev", "sess-aaaa1111", 1, "a-first"))
	m = feed(t, m, mkSandboxCtx("pix-dev", "sess-bbbb2222", 2, "b-first"))
	m = feed(t, m, mkSandboxCtx("pix-dev", "sess-aaaa1111", 3, "a-second"))
	m = feed(t, m, mkSandboxCtx("pix-dev", "sess-bbbb2222", 4, "b-second"))

	assertShape(t, m, []string{
		"node:" + sessionNodeID("pix-dev/sess-aaaa1111"), // primary = first-seen
		"row:pix-dev/sess-aaaa1111/ctx:1:1",
		"row:pix-dev/sess-aaaa1111/ctx:1:3",
		"node:" + sessionNodeID("pix-dev/sess-bbbb2222"), // child session
		"row:pix-dev/sess-bbbb2222/ctx:1:2",
		"row:pix-dev/sess-bbbb2222/ctx:1:4",
	})
...
func TestMonitorTUI_CollapseSessionNodes(t *testing.T) {
	...
	primaryNode := sessionNodeID("pix-dev/sess-aaaa1111")
	childNode := sessionNodeID("pix-dev/sess-bbbb2222")
...
func TestMonitorTUI_SingleSandboxNoTabBarNameInTitle(t *testing.T) {
	...
	if !strings.Contains(view, "pix monitor  solo-box  events=1") {
...
func TestMonitorTUI_SingleSessionStillRendersTree(t *testing.T) {
	...
	if !strings.Contains(m.headerLine(), "pix monitor  (local)  events=3") {
...
// toolInvokesPi must match `pi`/`pix` as a COMMAND TOKEN — anchored
