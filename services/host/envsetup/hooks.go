...
//     none of this.
//  4. Immediately before each execution, Run re-proves the executable on
//     disk still hashes to the accepted snapshot's SHA. That closes the
//     window between "a human said yes" and "we exec'd it" (TOCTOU): a
//     hook swapped after review is refused, not run.
//
...
// HashSetupExecutable is the ONE proof a setup hook's command is something
// this host may run: an existing, REGULAR (never a symlink, never a
// directory, never a device), executable file, returned as its sha256
// content hash. It is exported because the same proof has to be taken
// TWICE — once when the reviewed bill of materials is computed, and again
// immediately before the hook is executed — and the second proof would be
// worthless if it were a different, looser check than the first.
...
func runOne(dir string, h Hook, opts Options, ex Executor) (Outcome, error) {
	label := fmt.Sprintf("%s (%s)", h.ID, h.Kind)

	// TOCTOU re-proof: the accepted snapshot said this exact content. Take
	// the SAME proof ComputeBoM took (regular, non-symlink, executable,
	// hashable) against the disk as it is RIGHT NOW, and compare hashes.
	sha, err := HashSetupExecutable(h.Resolved)
	if err != nil {
		return soft(h, opts, label, fmt.Sprintf("%v", err))
	}
	...
	if sha != h.SHA {
		detail := fmt.Sprintf("%s changed on disk since it was reviewed (accepted sha256:%s, found sha256:%s)", h.Resolved, h.SHA, sha)
	...
	code, out, err := ex.Check(dir, h.Resolved, h.CheckArgs)
	if err != nil {
	...
	acode, aerr := ex.Apply(dir, h.Resolved, h.ApplyArgs, opts.In, opts.Out, opts.Err)
	if aerr != nil {
	...
	// present and working.
	pcode, pout, perr := ex.Check(dir, h.Resolved, h.CheckArgs)
	if perr != nil {
...
// keeps meaning "every required hook was probed ready".
...
// dumping a hook's whole (bounded) transcript into the error.
