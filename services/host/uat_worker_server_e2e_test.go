...
// This file was uat_mcp_test.go's original content before the U1
// session-worker transport split (docs/design/self-development-uat.md): an
// end-to-end exercise of workflow/uat.MCPServer's protocol handling
// (initialize, tools/list, tool calls, error codes, protocol purity) driven
// directly, the way pix-host uat-worker now drives it per accepted
// connection. It moved here, unmodified in substance, because uat_mcp.go
// is now the dumb relay and must never construct a Runner or import
// workflow/uat (see TestUatMcpGatewayIsADumbRelay in uat_mcp_test.go) — this
// coverage belongs conceptually with uat-worker, not the relay.
