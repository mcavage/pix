...
// observed it like a real statement and invented pi-stack facts/events from
// it. The fix is a marker contract: setup.go prefixes any generated message
// with "[pi-stack-generated:...] " and shouldCaptureUserText skips it.
import assert from "node:assert";
import { test } from "node:test";

const { shouldCaptureUserText } = await import("../extensions/memory-capture.ts");

test("a pi-stack-generated message (onboarding kickoff shape) is never captured", () => {
	assert.equal(
		shouldCaptureUserText(
			"[pi-stack-generated:onboarding] I just ran pi-stack setup — give me the guided walkthrough and help me get started.",
		),
		false,
	);
});

test("any [pi-stack-generated:...] prefixed message is skipped, not just onboarding", () => {
	assert.equal(shouldCaptureUserText("[pi-stack-generated:some-other-flow] hello there, this is long enough"), false);
});
...
test("a real message that merely mentions setup/onboarding is still captured (no broad skip)", () => {
	assert.equal(shouldCaptureUserText("I just ran pi-stack setup and it broke on step 3"), true);
});
